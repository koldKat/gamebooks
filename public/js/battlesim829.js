...
import { getPlayBtnRow } from './charsheet.js?v=39';
import { escapeHtml, registerPanelShortcut, shortcutLabel, ALL_PANEL_OVERLAY_IDS } from './util.js?v=20';
  ...
  return `
    <div class="inv-edit-row">
      ...
      <div class="autocomplete-wrap bsim-enemy-ac">
        <input id="bsim-enemy-name" class="inv-edit-input" type="text" autocomplete="off">
        <ul id="bsim-enemy-name-dropdown" class="autocomplete-dropdown"></ul>
      </div>
    </div>`;
}
...
function _setupEnemyAutocomplete() {
  ...
  function render(q) {
    ...
    dropdown.innerHTML = matches.map((e, i) =>
      `<li data-idx="${i}">${escapeHtml(e.name)}<span class="ac-sub">${escapeHtml(_enemyStatLabel(e))}</span></li>`
    ).join('');
    activeIdx = -1;
        ...
        <div class="bsim-col bsim-col-left">
          <div class="bsim-side">
            <div class="bsim-side-title">
              <span>Ти</span>
              <button id="bsim-sync-equip" class="bsim-sync-btn" type="button" title="Презареди А/З/ТЖ от екипировка и умения">⟳ От екипировка</button>
            </div>

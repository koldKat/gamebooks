...
import { render, openEditModal, closeEditModal, openNoteModal, closeNoteModal, showConfirm, showAlert, maxFastTravels, setFastTravelHandler, showFastTravelDialog, setOnTrailToggle, openPortalModal, setDiscoverableLimit, setOnChoicesRecorded, startPlaythrough, setAltStartHandler } from './play.js?v=36';
import { t, applyTranslations, setTranslationOverride } from './i18n.js?v=10';
import { initCharSheet, setCharSheetVisible, renderCharSheetDisplay } from './charsheet.js?v=28';
import { initInventory, setInventoryVisible, renderInventoryDisplay, preloadItems, setExtraDisplayItemsProvider } from './inventory.js?v=94';
import { initEquipment, setEquipmentVisible, getVisibleEquippedItems } from './equipment.js?v=81';
import { initNotes, hideNotesUI, loadNotesForBook, setOnXpAwarded as setNotesOnXpAwarded } from './notes.js?v=16';
import { initParty, connectPartySSE, disconnectPartySSE, loadPartyInvites, setPartyHooks } from './party.js?v=70';
import { initAuth, setOnAuthSuccess, showAuthForm, showResetPanel, hasPendingResetToken } from './auth.js?v=12';
import { initStats, closeStatsModal } from './stats.js?v=31';
import { setAddBookHooks, initAddBook, _closeAddBook, _closeAddComp, _closeAddSeries } from './add-book.js?v=73';
import {
...
import { initBattleSim, setBattleSimVisible, renderBattleSim } from './battlesim829.js?v=84';
import { initBattleSim8, setSim8Visible, renderSim8 } from './battlesim8.js?v=57';
import { initShop, updateCoinsDisplay, refreshCoinsDisplay, setShopHooks } from './shop.js?v=17';
import { initProfile, updateAvatarUI, renderBooksXpSummary, setProfileHooks } from './profile.js?v=34';
import { setPublicProfileHooks, closePublicModal, openPublicProfile, openPublicSeriesRun } from './public-profile.js?v=23';
import { setLiveTabHooks, _ensureLiveTabControllerStarted, _connectUserBadgeSSE, _disconnectUserBadgeSSE, _connectAppXpSSE, _disconnectAppXpSSE } from './livetab.js?v=23';
import { setAppXpHooks, refreshAppXp, handleAppXpEvent } from './app-xp.js?v=20';
import { setCoversHooks, loadCovers, openCoverActivity, openSeriesActivity, _showCachedCoversPanel, _refreshPublicCatalogIfVisible, _isLandingBooksViewVisible, _updateLandingBgDragUi, setCoversPrefsState, _toggleCoverTooltipSettings, initCoversPanel, resetFeedDisplayPrefsForLogout } from './covers.js?v=41';
import {
...
} from './open-world.js?v=66';
import { setFeedHooks, loadFeed, refreshDayCoverFlows } from './feed.js?v=56';
import {
...
import { initTips } from './tips.js?v=3';
import { initInbox } from './inbox.js?v=43';
import { initDice } from './dice.js?v=60';
import { initTooltip } from './tooltip.js?v=1';
import { exportAll, exportBook } from './export.js?v=41';
import { initFeedback } from './feedback.js?v=11';
import { setDemoHooks, getDemoBooks, setDemoBooks, getDemoVisited, startDemoMode, exitDemoMode, wasInDemoMode } from './demo.js?v=38';
import {
...
function showLogin() {
  ...
  _regBtn.textContent = 'Create Account';
...
async function showBooks() {
  ...
  if (isDemoMode) {
    ...
    document.getElementById('books-username').textContent = 'Demo';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('start-node-save').addEventListener('click', () => {
    ...
    if (!isValidSecId(newId) || (typeof newId === 'number' && newId < 1)) { errEl.textContent = 'Must be 1 or greater.'; return; }
    const oldId = ctxNodeId ?? state.startSection ?? 1;
    if (typeof newId === 'string' && !state.alphanumericSections) {
      showConfirm(
        `You entered an alphanumeric section ID ("${newId}"). Switch this book to alphanumeric mode? This cannot be undone.`,
        () => {
  ...
  document.getElementById('alt-start-save').addEventListener('click', () => {
    ...
    if (!isValidSecId(secId) || (typeof secId === 'number' && secId < 1)) { errEl.textContent = 'Must be 1 or greater.'; return; }
    if (typeof secId === 'string' && !state.alphanumericSections) {
      showConfirm(
        `You entered an alphanumeric section ID ("${secId}"). Switch this book to alphanumeric mode? This cannot be undone.`,
        () => {
          state.alphanumericSections = true;
          saveState();
          _startAltRun(secId);

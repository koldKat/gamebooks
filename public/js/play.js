...
import { network, visNodes, syncGraph } from './graph.js?v=68';
import { t } from './i18n.js?v=26';
import { renderCharSheetDisplay } from './charsheet.js?v=49';
import { naturalCompare } from './sort.js?v=1';
import { instantiateLoadout } from './equipment.js?v=102';
    ...
    if (!secData || secData.choices.length === 0) {
      ...
      html +=
        `<div class="input-group">` +
          ...
          `</div>` +
        `</div>`;
    } else if (secData.choices.length === 1 && !pt.path.includes(secData.choices[0]) && !_suppressAutoNav) {
...
export function navigate(sec) {
  ...
  const secData = state.graph[sec];
  const willAutoNav = !_suppressAutoNav && secData?.choices.length === 1 && !pt.path.includes(secData.choices[0]);
...
export function undoRun() {
  ...
  while (pt.path.length > 1) {
    ...
    const wouldAutoNav = !pt.path.includes(node.choices[0]);

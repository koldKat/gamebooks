// ── Battle Simulator (Замъкът на смъртта / Castle Death, Bulgarian
// edition of Lone Wolf book 7, id 435) ──
// Self-contained module. Imports from state.js, charsheet.js and util.js.
// Visibility is gated (book 435 only) by the caller in boot.js via
// setSim435Visible().
// To remove: delete this file, remove its import line and initSim435()/
// setSim435Visible() calls from boot.js, remove 'sim435' from
...
//
// English original is book 327 ("Castle Death"), which has no sim of its
// own yet (has_battle_sim=0), so nothing to cross-reference there. The
// Combat Ratio + Combat Results Table system and COMBAT_TABLE below are the
// same Lone Wolf combat mechanic used unchanged from
// battlesim118.js/battlesim322.js/battlesim430.js/battlesim431.js/
// battlesim432.js/battlesim434.js rather than re-derived.
//
...
// in this app.
//
// Most multi-enemy encounters in this book (e.g. §40/§50/§75/§206/§316/§319
// Захда зверове groups) are given a single combined БОЙНО УМЕНИЕ/
// ИЗДРЪЖЛИВОСТ stat line and fought as one enemy. §212 is the one exception
// - two dungeon guards are given separate stat lines ("Звяр тъмничар 1"/"2")
// and are fought sequentially, same as any other book's "re-pick the next
...
// УМЕНИЕ N, ИЗДРЪЖЛИВОСТ N" stat block), excluding §214's tunnel-door
// "battle" ("Тунелна врата", not a real enemy). Two dropped ИЗДРЪЖЛИВОСТ
// values (§40, §212 x2) found via the mandatory full prose read and
// restored from the raw PDF before extraction.
//
// All state lives in pt.sim435, per-user/per-book via currentPlaythrough().

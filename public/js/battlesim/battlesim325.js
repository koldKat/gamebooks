...
function _runRound() {
  ...
  _appendLog(d, t('battlesim325.log.round', { round: d.roundsThisBattle, ratio: d.ratio, pick }));
  ...
  _appendLog(d, t('battlesim325.log.result', {
    enemy: _enemyNameSafe(d),
    enemyLoss: enemyLoss === 'K' ? t('battlesim325.log.k_word') : enemyLoss,
    enemyEndurance: d.enemy.endurance, enemyEnduranceMax: d.enemy.enduranceMax,
    lwLoss: lwLoss === 'K' ? t('battlesim325.log.k_word') : lwLoss,
...
function _checkBattleEnd(d) {
  if (d.enemy.endurance <= 0) {
    _appendLog(d, t('battlesim325.log.defeated', { trophy: SVG_TROPHY, enemy: _enemyNameSafe(d) }));
    _recordOutcome(d, 'win');
  } else if (d.endurance <= 0) {
    _appendLog(d, t('battlesim325.log.fallen', { skull: SVG_SKULL }));
...
function _resetBattle() {
  ...
  if (d.log.length) _appendLog(d, t('battlesim325.log.reset_sep'));
  _appendLog(d, t('battlesim325.log.reset', { enemy: _enemyNameSafe(d) }));
  saveState();
  _renderAll();
}
...
function _usePotion() {
  ...
  if (d.roundsThisBattle > 0 && d.endurance > 0 && d.enemy.endurance > 0) {
    showAlert(t('battlesim325.alert.potion_midfight'));
    return;
  }
  d.healingPotionUsed = true;
  const before = d.endurance;
  d.endurance = Math.min(d.enduranceInitial, d.endurance + HEALING_POTION_HEAL);
  _appendLog(d, t('battlesim325.log.potion', { before, endurance: d.endurance, enduranceMax: d.enduranceInitial }));
  saveState();
  _renderAll();
}
...
function _renderInputs(skipEnemyPick) {
  const d = _data();
  if (!d) return;
  _setVal('sim325-cs', d.combatSkill);
  _setVal('sim325-csmax', d.combatSkillInitial);
  _setVal('sim325-en', d.endurance);
  _setVal('sim325-enmax', d.enduranceInitial);
  _setVal('sim325-atkmod', d.attackModifier);
  _setVal('sim325-enemy-skill', d.enemy.skill);
  _setVal('sim325-enemy-en', d.enemy.endurance);
  _setVal('sim325-enemy-enmax', d.enemy.enduranceMax);
  _setVal('sim325-ratio', d.ratio);
  if (!skipEnemyPick) _setVal('sim325-enemy-pick', d.enemy.name);

  const potionBtn = document.getElementById('sim325-use-potion');
  potionBtn.disabled = !d.rolled || !d.hasHealingPotion || d.healingPotionUsed;
  potionBtn.textContent = d.healingPotionUsed ? t('battlesim325.btn.used') : t('battlesim325.btn.drink');

  const rollBtn = document.getElementById('sim325-roll');
  rollBtn.disabled = d.rolled;
  rollBtn.textContent = d.rolled ? t('battlesim325.btn.rolled') : t('battlesim325.btn.roll');

  const status = document.getElementById('sim325-status');
  if (!d.rolled) {
    status.textContent = t('battlesim325.status.not_ready');
  } else if (d.endurance <= 0) {
    status.textContent = t('battlesim325.status.fallen');
  } else if (d.enemy.endurance <= 0 && d.enemy.enduranceMax > 0) {
    status.textContent = t('battlesim325.status.defeated', { enemy: _enemyName(d) });
  } else {
    status.textContent = '';
  }
  document.getElementById('sim325-round').disabled = !d.rolled || d.endurance <= 0 || d.enemy.endurance <= 0;
}

function _renderLog() {
  const d = _data();
  const el = document.getElementById('sim325-log');
  if (!el || !d) return;
...
function _renderHistory() {
  const d      = _data();
  const sumEl  = document.getElementById('sim325-history-summary');
  const listEl = document.getElementById('sim325-history-list');
  if (!d || !sumEl || !listEl) return;
  sumEl.textContent = t('battlesim325.history.summary', { n: d.history.length });
  if (!d.history.length) {
    listEl.innerHTML = `<div class="bsim-history-empty">${t('battlesim325.history.empty')}</div>`;
    return;
  }
  listEl.innerHTML = d.history.slice().reverse().map(h => {
    const icon   = h.outcome === 'win' ? SVG_TROPHY : SVG_SKULL;
    const result = h.outcome === 'win' ? t('battlesim325.history.won') : t('battlesim325.history.lost');
...
export function renderSim325() {
  const overlay = document.getElementById('sim325-overlay');
  if (!overlay || !overlay.classList.contains('active')) return;
  if (!_data()) { closeSim325(); return; }
  _renderAll();
}
...
function openSim325() {
  ...
  _renderAll();
  document.getElementById('sim325-overlay').classList.add('active');
}

function closeSim325() {
  document.getElementById('sim325-overlay')?.classList.remove('active');
}

export function setSim325Visible(visible) {
  const btn = document.getElementById('sim325-btn');
  if (btn) btn.style.display = visible ? '' : 'none';
  if (!visible) closeSim325();
}
...
export function initSim325() {
  const overlay = document.createElement('div');
  overlay.id        = 'sim325-overlay';
  overlay.className = 'inv-overlay';
  overlay.innerHTML = `
    <div class="inv-modal bsim-modal">
      <div class="inv-modal-hdr">
        <span class="inv-modal-title">${t('battlesim325.ui.title')}</span>
        <button id="sim325-close" class="inv-close-btn" aria-label="${t('btn.close')}">✕</button>
      </div>
      <div class="bsim-body">
        <div class="bsim-col bsim-col-left">
          <div class="bsim-side">
            <div class="inv-edit-row bsim-life-roll-row">
              <button id="sim325-roll" class="inv-edit-done bsim-ae-roll-btn" type="button">${t('battlesim325.btn.roll')}</button>
            </div>
            ${_numField(t('battlesim325.ui.cs'), 'sim325-cs')}
            ${_numField(t('battlesim325.ui.cs_initial'), 'sim325-csmax')}
            ${_numField(t('battlesim325.ui.en'), 'sim325-en')}
            ${_numField(t('battlesim325.ui.en_initial'), 'sim325-enmax')}
            ${_numField(t('battlesim325.ui.atkmod'), 'sim325-atkmod')}
            <div class="inv-edit-row">
              <span class="inv-edit-label bsim-stat-label">${t('battlesim325.ui.potion')}</span>
              <button id="sim325-use-potion" class="inv-edit-done bsim-ae-roll-btn" type="button">${t('battlesim325.btn.drink')}</button>
            </div>
          </div>
          <div class="bsim-side">
            <div class="inv-edit-row">
              <span class="inv-edit-label bsim-stat-label">${t('battlesim325.ui.pick')}</span>
              <div class="autocomplete-wrap bsim-enemy-ac">
                <input id="sim325-enemy-pick" class="inv-edit-input" type="text" autocomplete="off" readonly role="combobox" aria-autocomplete="list" aria-expanded="false" aria-haspopup="listbox" aria-controls="sim325-enemy-pick-dropdown">
                <ul id="sim325-enemy-pick-dropdown" class="autocomplete-dropdown" role="listbox"></ul>
              </div>
            </div>
            ${_numField(t('battlesim325.ui.enemy_cs'), 'sim325-enemy-skill')}
            ${_numField(t('battlesim325.ui.enemy_en'), 'sim325-enemy-en')}
            ${_numField(t('battlesim325.ui.enemy_en_max'), 'sim325-enemy-enmax')}
            ${_numField(t('battlesim325.ui.ratio'), 'sim325-ratio', null, true)}
          </div>
          <div id="sim325-status" class="bsim-status"></div>
          <div class="inv-modal-ftr">
            <button id="sim325-round" class="inv-add-btn bsim-action-primary">${t('battlesim325.btn.round')}</button>
            <button id="sim325-reset" class="inv-add-btn">${t('battlesim325.btn.reset')}</button>
          </div>
        </div>
        <div class="bsim-col bsim-col-right">
          <details class="bsim-history">
            <summary id="sim325-history-summary">${t('battlesim325.history.summary', { n: 0 })}</summary>
            <div id="sim325-history-list" class="bsim-history-list"></div>
          </details>
          <div id="sim325-log" class="bsim-log"></div>
        </div>
      </div>
    </div>`;
  ...
  btn.addEventListener('click', openSim325);
  document.getElementById('sim325-close').addEventListener('click', closeSim325);
  let _mdOnOverlay = false;
  overlay.addEventListener('mousedown', e => { _mdOnOverlay = e.target === overlay; });
  overlay.addEventListener('click', e => { if (e.target === overlay && _mdOnOverlay) closeSim325(); });
  registerPanelShortcut('KeyS', {
    getButton:  () => btn,
    getOverlay: () => overlay,
    otherOverlayIds: ALL_PANEL_OVERLAY_IDS.filter(id => id !== 'sim325-overlay'),
    open:  openSim325,
    close: closeSim325,
  });
  document.addEventListener('keydown', e => {
    if (e.key === 'Escape' && overlay.classList.contains('active')) closeSim325();
  });

  document.getElementById('sim325-round').addEventListener('click', _runRound);
  document.getElementById('sim325-reset').addEventListener('click', _resetBattle);
  document.getElementById('sim325-use-potion').addEventListener('click', _usePotion);

  document.getElementById('sim325-roll').addEventListener('click', () => {
    const d = _data();
    ...
    _appendLog(d, t('battlesim325.log.rolled', { cs: d.combatSkillInitial, en: d.enduranceInitial }));
    saveState();
    _renderAll();
  });

  document.getElementById('sim325-enemy-pick').addEventListener('input', e => {
    const d = _data();
    if (!d) return;
    d.enemy.name = e.target.value;
    saveState();
  });
  _setupAutocomplete('sim325-enemy-pick', 'sim325-enemy-pick-dropdown', enemy => {
    const d = _data();
    if (!d) return;
  ...
  const fieldMap = {
    'sim325-cs': ['combatSkill'], 'sim325-csmax': ['combatSkillInitial'],
    'sim325-en': ['endurance'], 'sim325-enmax': ['enduranceInitial'],
    'sim325-atkmod': ['attackModifier'],
    'sim325-enemy-skill': ['enemy', 'skill'], 'sim325-enemy-en': ['enemy', 'endurance'], 'sim325-enemy-enmax': ['enemy', 'enduranceMax'],
  };
  for (const [id, path] of Object.entries(fieldMap)) {
    ...
    input.addEventListener('change', () => {
      const d = _data();
      if (!d) return;
      const allowNegative = id === 'sim325-atkmod';
      ...
      if (id === 'sim325-cs' || id === 'sim325-atkmod' || id === 'sim325-enemy-skill') d.ratio = _effectiveSkill(d) - d.enemy.skill;
      saveState();
      _renderInputs(true);
    });
  }

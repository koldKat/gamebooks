// ── Battle Simulator (Царствата на терора / Kingdoms of Terror, Bulgarian
// edition of Lone Wolf book 6, id 434) ──
// Self-contained module. Imports from state.js, charsheet.js and util.js.
// Visibility is gated (book 434 only) by the caller in boot.js via
// setSim434Visible().
// To remove: delete this file, remove its import line and initSim434()/
// setSim434Visible() calls from boot.js, remove 'sim434' from
...
//
// English original is book 326 ("The Kingdoms of Terror"), which has no sim
...
// battlesim432.js rather than re-derived.
//
...
// in this app.
//
// Every multi-enemy encounter in this book (e.g. §159 three mercenaries,
// §254/§270/§282/§337 groups of thieves/pirates/undead) is given a single
// combined БОЙНО УМЕНИЕ/ИЗДРЪЖЛИВОСТ stat line and fought as one enemy per
// the book's own text - no pairedFight/sideEnemy mechanic needed.
//
// book_enemies.attack holds БОЙНО УМЕНИЕ, .hp holds ИЗДРЪЖЛИВОСТ, .defense
// unused - same convention as every other Lone Wolf sim. 26 rows extracted
// directly from this book's own section text (regex on every "Name: БОЙНО
// УМЕНИЕ N, ИЗДРЪЖЛИВОСТ N" stat block), excluding two non-combat entries
// that don't fit the pattern: §26's archery-tournament target score
// ("ИЗДРЪЖЛИВОСТ (точки на мишената)") and §156's canal-door "battle"
// ("ИЗДРЪЖЛИВОСТ (точки УСТОЙЧИВОСТ)", not a real enemy).
//
// All state lives in pt.sim434, per-user/per-book via currentPlaythrough().

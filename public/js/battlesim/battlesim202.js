...
function _runRound() {
  ...
  if (enemyInstaKill) {
    _appendLog(d, `${_enemyNameSafe(d)}'s attack totals ${enemyAS} - the blow is fatal.`);
    d.player.stamina = 0;
  ...
  if (playerAS === enemyAS) {
    ...
    if (d.player.luckyKillOnWin) {
      ...
      _appendLog(d, `You land a blow on ${_enemyNameSafe(d)} - Test Your Luck to see if you've found its weak point.`);
    } else {
      ...
      if (d.player.winAfterHits > 0 && d.player.hitsLandedThisFight >= d.player.winAfterHits && d.enemy.stamina > 0) {
        d.enemy.stamina = 0;
        _appendLog(d, `You've landed enough blows to find the weak point - the killing strike goes home.`);
      }
      if (d.enemy.stamina > 0) d.pendingLuckQueue.push({ kind: 'player-hit' });
    }
  } else if (d.player.instaKillOnEnemyWin) {
    _appendLog(d, `${_enemyNameSafe(d)} wins the Attack Round - the result is immediately fatal.`);
    d.player.stamina = 0;
  } else {
  ...
  if (d.pairedFight && d.sideEnemy.staminaMax > 0 && d.player.stamina > 0) {
    ...
    if (sideInstaKill) {
      _appendLog(d, `${_sideEnemyNameSafe(d)}'s attack totals ${sideAS} - the blow is fatal.`);
      d.player.stamina = 0;
...
function _testLuck() {
  ...
  if (event.kind === 'weakpoint-hit') {
    if (lucky) {
      d.enemy.stamina = 0;
      _appendLog(d, `Test Your Luck: ${roll} (Lucky) - you've found the weak point! ${_enemyNameSafe(d)} is defeated.`);
      _appendLog(d, `${SVG_TROPHY} ${_enemyNameSafe(d)} is defeated!`);
      _recordOutcome(d, 'win');
    } else {
      _appendLog(d, `Test Your Luck: ${roll} (Unlucky) - the wound has no effect. STAMINA: ${d.enemy.stamina}/${d.enemy.staminaMax}.`);
    }
...
function _skipLuck() {
  ...
  if (event.kind === 'weakpoint-hit') {
    _appendLog(d, `You decide not to press for the weak point. STAMINA: ${d.enemy.stamina}/${d.enemy.staminaMax}.`);
  }
  saveState();
  _renderAll();
}
...
function _renderItemsHtml(d) {
  return `
    <div class="bsim-tech-row">
      <div class="bsim-tech-name">Dwarf Chainmail <span class="bsim-tech-uses">(sec. 28)</span></div>
      <div class="bsim-tech-desc">+1 SKILL while worn.</div>
      <div class="bsim-tech-footer"><label class="inv-edit-check-label"><input type="checkbox" id="sim202-item-chainmail" class="inv-edit-check" ${d.player.hasChainmail ? 'checked' : ''}> Have it</label></div>
    </div>
    <div class="bsim-tech-row">
      <div class="bsim-tech-name">Iron Shield <span class="bsim-tech-uses">(sec. 95)</span></div>
      <div class="bsim-tech-desc">+1 SKILL while carried. Also fully blocks the Manticore's tail-spike volley on that route.</div>
      <div class="bsim-tech-footer"><label class="inv-edit-check-label"><input type="checkbox" id="sim202-item-ironshield" class="inv-edit-check" ${d.player.hasIronShield ? 'checked' : ''}> Have it</label></div>
    </div>
    <div class="bsim-tech-row">
      <div class="bsim-tech-name">Ninja Curved Sword <span class="bsim-tech-uses">(sec. 286)</span></div>
      <div class="bsim-tech-desc">+4 SKILL while wielded - replaces your old weapon's bonus, only one weapon bonus applies at a time.</div>
      <div class="bsim-tech-footer"><label class="inv-edit-check-label"><input type="checkbox" id="sim202-item-ninjasword" class="inv-edit-check" ${d.player.hasNinjaSword ? 'checked' : ''}> Have it</label></div>
    </div>
    <div class="bsim-tech-row">
      <div class="bsim-tech-name">Winged Helmet <span class="bsim-tech-uses">(sec. 218)</span></div>
      <div class="bsim-tech-desc">+1 SKILL while worn.</div>
      <div class="bsim-tech-footer"><label class="inv-edit-check-label"><input type="checkbox" id="sim202-item-wingedhelmet" class="inv-edit-check" ${d.player.hasWingedHelmet ? 'checked' : ''}> Have it</label></div>
    </div>`;
}
...
function _renderInputs() {
  ...
  document.getElementById('sim202-potion-uses').textContent = `${d.player.potionUsesLeft} use(s) left`;
      ...
      <div class="bsim-body">
        <div class="bsim-col bsim-col-left">
          <div class="bsim-side">
            ...
            <div class="inv-edit-row bsim-life-roll-row">
              <button id="sim202-roll" class="inv-edit-done bsim-ae-roll-btn" type="button">Roll starting SKILL/STAMINA/LUCK</button>
            </div>
            ${_numField('SKILL', 'sim202-player-skill')}
            ${_numField('Initial SKILL', 'sim202-player-skillmax')}
            ${_numField('STAMINA', 'sim202-player-stamina')}
            ${_numField('Initial STAMINA', 'sim202-player-staminamax')}
            ${_numField('LUCK', 'sim202-player-luck')}
            ${_numField('Initial LUCK', 'sim202-player-luckmax')}
            ${_numField('Attack modifier', 'sim202-player-atkmod')}
            <div class="inv-edit-row">
            ...
            <div class="inv-edit-row bsim-ae-row">
              ...
              <button id="sim202-potion-use" class="inv-edit-done bsim-ae-roll-btn" type="button">Drink</button>
            </div>
            <div class="inv-edit-row bsim-ae-row">
              <span class="inv-edit-label bsim-stat-label">Provisions</span>
              <span id="sim202-provisions-left" class="bsim-ae-display"></span>
              <button id="sim202-provisions" class="inv-edit-done bsim-ae-roll-btn" type="button">Eat (+4 STAMINA)</button>
            </div>
          </div>
          <div class="bsim-side">
            ...
            ${_numField('SKILL', 'sim202-enemy-skill')}
            ${_numField('STAMINA', 'sim202-enemy-stamina')}
            ${_numField('Max STAMINA', 'sim202-enemy-staminamax')}
            ${_numField('Wound damage', 'sim202-enemy-wounddmg')}
            ${_numField('Instant-death Attack Strength (0=off)', 'sim202-enemy-askill')}
            ${_numField('Win after N landed hits (0=off)', 'sim202-enemy-winhits')}
            <div class="inv-edit-row">
              <label class="inv-edit-check-label"><input type="checkbox" id="sim202-insta-win" class="inv-edit-check"> Enemy winning any round is instantly fatal</label>
            </div>
            <div class="inv-edit-row">
              <label class="inv-edit-check-label"><input type="checkbox" id="sim202-lucky-kill" class="inv-edit-check"> Every landed hit requires a Luck test to find the weak point</label>
            </div>
            <div class="inv-edit-row">
              <label class="inv-edit-check-label"><input type="checkbox" id="sim202-paired" class="inv-edit-check"> Second attacker fights alongside (never woundable)</label>
            </div>
            <div id="sim202-side-fields" style="display:none">
              ...
              ${_numField('SKILL', 'sim202-side-skill')}
              ${_numField('Max STAMINA', 'sim202-side-staminamax')}
            </div>
          </div>
          ...
          <div id="sim202-luck-prompt" class="inv-edit-row bsim-heal-row" style="display:none">
            ...
            <button id="sim202-luck-yes" class="inv-edit-done bsim-heal-btn" type="button">Test Luck</button>
            <button id="sim202-luck-no" class="inv-edit-done bsim-heal-btn" type="button">Skip</button>
          </div>
          <div class="inv-modal-ftr">
            <button id="sim202-round" class="inv-add-btn bsim-action-primary">Round</button>
            <button id="sim202-reset" class="inv-add-btn">Reset</button>
          </div>
        </div>
        <div class="bsim-col bsim-col-right">
          ...
          <details class="bsim-history">
            <summary id="sim202-history-summary">Battle History (0)</summary>

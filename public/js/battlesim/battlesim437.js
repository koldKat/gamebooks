// ── Battle Simulator (Котелът на страха / The Cauldron of Fear, Bulgarian
// edition of Lone Wolf book 9, id 437) ──
// Self-contained module. Imports from state.js, charsheet.js and util.js.
// Visibility is gated (book 437 only) by the caller in boot.js via
// setSim437Visible().
// To remove: delete this file, remove its import line and initSim437()/
// setSim437Visible() calls from boot.js, remove 'sim437' from
...
//
// English original is book 329 ("The Cauldron of Fear"), which has no sim
...
// battlesim432.js/battlesim434.js/battlesim435.js/battlesim436.js rather
// than re-derived.
//
...
// in this app.
//
// Most multi-enemy encounters in this book (e.g. §98/§127/§308 Делденийски
// наемници, §148 six wolf-riders) are given a single combined БОЙНО
// УМЕНИЕ/ИЗДРЪЖЛИВОСТ stat line and fought as one enemy. §238/§309 (a
// sergeant plus multiple paired tower guards) are the exception - several
// separate stat lines fought sequentially, same as any other book's
// "re-pick the next roster enemy after defeating the current one" pattern
// - no special code needed, just separate book_enemies rows.
//
// This book's own equipment rules offer the same three distinct healing
// items seen in books 434-436 - Отвара от лаумспур (+4), Еликсирът на
// Рендалим (+6), Цвят от оксидин (+2). Only Лаумспур is modeled here as
// the sim's single post-battle heal slot, matching every other Lone Wolf
// sim in this app; the other two are real per-book items but not
// simulated, same precedent as un-modeled non-combat minigames elsewhere.
//
// book_enemies.attack holds БОЙНО УМЕНИЕ, .hp holds ИЗДРЪЖЛИВОСТ, .defense
// unused - same convention as every other Lone Wolf sim. 42 rows extracted
...
// (§3, §119, §129, §182, §238) found via the mandatory full prose read and
// restored from the raw PDF before extraction.
//
// All state lives in pt.sim437, per-user/per-book via currentPlaythrough().
...
function _runRound() {
  ...
  _appendLog(d, t('battlesim437.log.result', {
    ...
    enemyLoss: enemyLoss === 'K' ? t('battlesim437.log.k_word') : enemyLoss,
    enemyEndurance: d.enemy.endurance, enemyEnduranceMax: d.enemy.enduranceMax,
    lwLoss: lwLoss === 'K' ? t('battlesim437.log.k_word') : lwLoss,
...
function _usePotion() {
  ...
  _appendLog(d, t('battlesim437.log.potion', { before, endurance: d.endurance, enduranceMax: d.enduranceInitial }));
  saveState();
  _renderAll();
}
...
function _renderInputs(skipEnemyPick) {
  const d = _data();
  if (!d) return;
  _setVal('sim437-cs', d.combatSkill);
  _setVal('sim437-csmax', d.combatSkillInitial);
  ...
  rollBtn.textContent = d.rolled ? t('battlesim437.btn.rolled') : t('battlesim437.btn.roll');

  const status = document.getElementById('sim437-status');
  if (!d.rolled) {
...
function _renderLog() {
  const d = _data();
  const el = document.getElementById('sim437-log');
  if (!el || !d) return;
...
function _renderHistory() {
  const d      = _data();
  const sumEl  = document.getElementById('sim437-history-summary');
  const listEl = document.getElementById('sim437-history-list');
  if (!d || !sumEl || !listEl) return;
  sumEl.textContent = t('battlesim437.history.summary', { n: d.history.length });
  if (!d.history.length) {
    listEl.innerHTML = `<div class="bsim-history-empty">${t('battlesim437.history.empty')}</div>`;
    return;
  }
  listEl.innerHTML = d.history.slice().reverse().map(h => {
    const icon   = h.outcome === 'win' ? SVG_TROPHY : SVG_SKULL;
    const result = h.outcome === 'win' ? t('battlesim437.history.won') : t('battlesim437.history.lost');
...
export function renderSim437() {
  const overlay = document.getElementById('sim437-overlay');
  if (!overlay || !overlay.classList.contains('active')) return;
  if (!_data()) { closeSim437(); return; }
  _renderAll();
}
...
function openSim437() {
  ...
  _renderAll();
  document.getElementById('sim437-overlay').classList.add('active');
}

function closeSim437() {
  document.getElementById('sim437-overlay')?.classList.remove('active');
}

export function setSim437Visible(visible) {
  const btn = document.getElementById('sim437-btn');
  if (btn) btn.style.display = visible ? '' : 'none';
  if (!visible) closeSim437();
}
...
export function initSim437() {
  const overlay = document.createElement('div');
  overlay.id        = 'sim437-overlay';
  overlay.className = 'inv-overlay';
  overlay.innerHTML = `
    <div class="inv-modal bsim-modal">
      <div class="inv-modal-hdr">
        <span class="inv-modal-title">${t('battlesim437.ui.title')}</span>
        <button id="sim437-close" class="inv-close-btn" aria-label="${t('btn.close')}">✕</button>
      </div>
      <div class="bsim-body">
        <div class="bsim-col bsim-col-left">
          <div class="bsim-side">
            <div class="inv-edit-row bsim-life-roll-row">
              <button id="sim437-roll" class="inv-edit-done bsim-ae-roll-btn" type="button">${t('battlesim437.btn.roll')}</button>
            </div>
            ${_numField(t('battlesim437.ui.cs'), 'sim437-cs')}
            ${_numField(t('battlesim437.ui.cs_initial'), 'sim437-csmax')}
          ...
          <div class="bsim-side">
            <div class="inv-edit-row">
              <span class="inv-edit-label bsim-stat-label">${t('battlesim437.ui.pick')}</span>
              <div class="autocomplete-wrap bsim-enemy-ac">
                <input id="sim437-enemy-pick" class="inv-edit-input" type="text" autocomplete="off" readonly role="combobox" aria-autocomplete="list" aria-expanded="false" aria-haspopup="listbox" aria-controls="sim437-enemy-pick-dropdown">
                <ul id="sim437-enemy-pick-dropdown" class="autocomplete-dropdown" role="listbox"></ul>
              </div>
            </div>
            ${_numField(t('battlesim437.ui.enemy_cs'), 'sim437-enemy-skill')}
            ${_numField(t('battlesim437.ui.enemy_en'), 'sim437-enemy-en')}
            ${_numField(t('battlesim437.ui.enemy_en_max'), 'sim437-enemy-enmax')}
            ${_numField(t('battlesim437.ui.ratio'), 'sim437-ratio', null, true)}
          </div>
        ...
        <div class="bsim-col bsim-col-right">
          <details class="bsim-history">
            <summary id="sim437-history-summary">${t('battlesim437.history.summary', { n: 0 })}</summary>
            <div id="sim437-history-list" class="bsim-history-list"></div>
          </details>
          <div id="sim437-log" class="bsim-log"></div>
        </div>
      </div>
    </div>`;
  ...
  btn.addEventListener('click', openSim437);
  document.getElementById('sim437-close').addEventListener('click', closeSim437);
  let _mdOnOverlay = false;
  overlay.addEventListener('mousedown', e => { _mdOnOverlay = e.target === overlay; });
  overlay.addEventListener('click', e => { if (e.target === overlay && _mdOnOverlay) closeSim437(); });
  registerPanelShortcut('KeyS', {
    getButton:  () => btn,
    getOverlay: () => overlay,
    otherOverlayIds: ALL_PANEL_OVERLAY_IDS.filter(id => id !== 'sim437-overlay'),
    open:  openSim437,
    close: closeSim437,
  });
  document.addEventListener('keydown', e => {
    if (e.key === 'Escape' && overlay.classList.contains('active')) closeSim437();
  });
  ...
  document.getElementById('sim437-roll').addEventListener('click', () => {
    ...
    _appendLog(d, t('battlesim437.log.rolled', { cs: d.combatSkillInitial, en: d.enduranceInitial }));
    saveState();
    _renderAll();
  });

  document.getElementById('sim437-enemy-pick').addEventListener('input', e => {
    const d = _data();
    if (!d) return;
    d.enemy.name = e.target.value;
    saveState();
  });
  _setupAutocomplete('sim437-enemy-pick', 'sim437-enemy-pick-dropdown', enemy => {
    const d = _data();
    if (!d) return;
  ...
  const fieldMap = {
    'sim437-cs': ['combatSkill'], 'sim437-csmax': ['combatSkillInitial'],
    'sim437-en': ['endurance'], 'sim437-enmax': ['enduranceInitial'],
    'sim437-atkmod': ['attackModifier'],
    'sim437-enemy-skill': ['enemy', 'skill'], 'sim437-enemy-en': ['enemy', 'endurance'], 'sim437-enemy-enmax': ['enemy', 'enduranceMax'],
  };
  for (const [id, path] of Object.entries(fieldMap)) {
    ...
    input.addEventListener('change', () => {
      const d = _data();
      if (!d) return;
      const allowNegative = id === 'sim437-atkmod';
      ...
      if (id === 'sim437-cs' || id === 'sim437-atkmod' || id === 'sim437-enemy-skill') d.ratio = _effectiveSkill(d) - d.enemy.skill;
      saveState();
      _renderInputs(true);
    });
  }

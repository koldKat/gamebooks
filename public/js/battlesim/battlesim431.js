// ── Battle Simulator (Пещерите на Калте / The Caverns of Kalte, Bulgarian
// edition of Lone Wolf book 3, id 431) ──
// Self-contained module. Imports from state.js, charsheet.js and util.js.
// Visibility is gated (book 431 only) by the caller in boot.js via
// setSim431Visible().
// To remove: delete this file, remove its import line and initSim431()/
// setSim431Visible() calls from boot.js, remove 'sim431' from
...
//
// This is the SAME underlying book as battlesim323.js (book 323, the English
// Project Aon "The Caverns of Kalte") - a separate, independently created
// Bulgarian translation (id 431), not a duplicate import. Section numbering
// matches 1:1 between the two editions (spot-checked against several
// sections), so the Combat Ratio + Combat Results Table system and
// COMBAT_TABLE below are reused unchanged from battlesim118.js/
// battlesim322.js/battlesim430.js rather than re-derived.
//
...
// fights (§32/§263 Kalkoth trio, §89/§343 Doomwolf trio) are represented as
// separate book_enemies rows each, matching book 323's own precedent.
//
// book_enemies.attack holds БОЙНИ УМЕНИЯ, .hp holds ИЗДРЪЖЛИВОСТ, .defense
// unused - same convention as every other Lone Wolf sim. 38 rows, directly
// cross-referenced against book 323's already-verified roster (same
// numbers at every matching section number) with Bulgarian names read from
// this book's own section text.
//
// All state lives in pt.sim431, per-user/per-book via currentPlaythrough().
...
function _runRound() {
  ...
  _appendLog(d, t('battlesim431.log.result', {
    enemy: _enemyNameSafe(d),
    enemyLoss: enemyLoss === 'K' ? t('battlesim431.log.k_word') : enemyLoss,
    enemyEndurance: d.enemy.endurance, enemyEnduranceMax: d.enemy.enduranceMax,
    lwLoss: lwLoss === 'K' ? t('battlesim431.log.k_word') : lwLoss,
...
function _resetBattle() {
  ...
  _appendLog(d, t('battlesim431.log.reset', { enemy: _enemyNameSafe(d) }));
  saveState();
  _renderAll();
}
...
function _usePotion() {
  ...
  _appendLog(d, t('battlesim431.log.potion', { before, endurance: d.endurance, enduranceMax: d.enduranceInitial }));
  saveState();
  _renderAll();
}
...
function _renderInputs(skipEnemyPick) {
  const d = _data();
  if (!d) return;
  _setVal('sim431-cs', d.combatSkill);
  _setVal('sim431-csmax', d.combatSkillInitial);
  _setVal('sim431-en', d.endurance);
  _setVal('sim431-enmax', d.enduranceInitial);
  _setVal('sim431-atkmod', d.attackModifier);
  _setVal('sim431-enemy-skill', d.enemy.skill);
  _setVal('sim431-enemy-en', d.enemy.endurance);
  _setVal('sim431-enemy-enmax', d.enemy.enduranceMax);
  _setVal('sim431-ratio', d.ratio);
  ...
  rollBtn.textContent = d.rolled ? t('battlesim431.btn.rolled') : t('battlesim431.btn.roll');

  const status = document.getElementById('sim431-status');
  if (!d.rolled) {
    status.textContent = t('battlesim431.status.not_ready');
  } else if (d.endurance <= 0) {
    status.textContent = t('battlesim431.status.fallen');
  } else if (d.enemy.endurance <= 0 && d.enemy.enduranceMax > 0) {
    status.textContent = t('battlesim431.status.defeated', { enemy: _enemyName(d) });
  } else {
    status.textContent = '';
  }
  document.getElementById('sim431-round').disabled = !d.rolled || d.endurance <= 0 || d.enemy.endurance <= 0;
}

function _renderLog() {
  const d = _data();
  const el = document.getElementById('sim431-log');
  if (!el || !d) return;
...
function _renderHistory() {
  const d      = _data();
  const sumEl  = document.getElementById('sim431-history-summary');
  const listEl = document.getElementById('sim431-history-list');
  if (!d || !sumEl || !listEl) return;
  sumEl.textContent = t('battlesim431.history.summary', { n: d.history.length });
  if (!d.history.length) {
    listEl.innerHTML = `<div class="bsim-history-empty">${t('battlesim431.history.empty')}</div>`;
    return;
  }
  listEl.innerHTML = d.history.slice().reverse().map(h => {
    const icon   = h.outcome === 'win' ? SVG_TROPHY : SVG_SKULL;
    const result = h.outcome === 'win' ? t('battlesim431.history.won') : t('battlesim431.history.lost');
...
export function renderSim431() {
  const overlay = document.getElementById('sim431-overlay');
  if (!overlay || !overlay.classList.contains('active')) return;
  if (!_data()) { closeSim431(); return; }
  _renderAll();
}
...
function openSim431() {
  ...
  _renderAll();
  document.getElementById('sim431-overlay').classList.add('active');
}

function closeSim431() {
  document.getElementById('sim431-overlay')?.classList.remove('active');
}

export function setSim431Visible(visible) {
  const btn = document.getElementById('sim431-btn');
  if (btn) btn.style.display = visible ? '' : 'none';
  if (!visible) closeSim431();
}
...
export function initSim431() {
  const overlay = document.createElement('div');
  overlay.id        = 'sim431-overlay';
  overlay.className = 'inv-overlay';
  overlay.innerHTML = `
    <div class="inv-modal bsim-modal">
      <div class="inv-modal-hdr">
        <span class="inv-modal-title">${t('battlesim431.ui.title')}</span>
        <button id="sim431-close" class="inv-close-btn" aria-label="${t('btn.close')}">✕</button>
      </div>
      <div class="bsim-body">
        <div class="bsim-col bsim-col-left">
          <div class="bsim-side">
            <div class="inv-edit-row bsim-life-roll-row">
              <button id="sim431-roll" class="inv-edit-done bsim-ae-roll-btn" type="button">${t('battlesim431.btn.roll')}</button>
            </div>
            ${_numField(t('battlesim431.ui.cs'), 'sim431-cs')}
            ${_numField(t('battlesim431.ui.cs_initial'), 'sim431-csmax')}
            ${_numField(t('battlesim431.ui.en'), 'sim431-en')}
            ${_numField(t('battlesim431.ui.en_initial'), 'sim431-enmax')}
            ${_numField(t('battlesim431.ui.atkmod'), 'sim431-atkmod')}
            <div class="inv-edit-row">
              <span class="inv-edit-label bsim-stat-label">${t('battlesim431.ui.potion')}</span>
              <button id="sim431-use-potion" class="inv-edit-done bsim-ae-roll-btn" type="button">${t('battlesim431.btn.drink')}</button>
            </div>
          </div>
          <div class="bsim-side">
            <div class="inv-edit-row">
              <span class="inv-edit-label bsim-stat-label">${t('battlesim431.ui.pick')}</span>
              <div class="autocomplete-wrap bsim-enemy-ac">
                <input id="sim431-enemy-pick" class="inv-edit-input" type="text" autocomplete="off" readonly role="combobox" aria-autocomplete="list" aria-expanded="false" aria-haspopup="listbox" aria-controls="sim431-enemy-pick-dropdown">
                <ul id="sim431-enemy-pick-dropdown" class="autocomplete-dropdown" role="listbox"></ul>
              </div>
            </div>
            ${_numField(t('battlesim431.ui.enemy_cs'), 'sim431-enemy-skill')}
            ${_numField(t('battlesim431.ui.enemy_en'), 'sim431-enemy-en')}
          ...
          <div class="inv-modal-ftr">
            ...
            <button id="sim431-reset" class="inv-add-btn">${t('battlesim431.btn.reset')}</button>
          </div>
        </div>
        <div class="bsim-col bsim-col-right">
          <details class="bsim-history">
            <summary id="sim431-history-summary">${t('battlesim431.history.summary', { n: 0 })}</summary>
            <div id="sim431-history-list" class="bsim-history-list"></div>
          </details>
          <div id="sim431-log" class="bsim-log"></div>
        </div>
      </div>
    </div>`;
  ...
  btn.addEventListener('click', openSim431);
  document.getElementById('sim431-close').addEventListener('click', closeSim431);
  let _mdOnOverlay = false;
  overlay.addEventListener('mousedown', e => { _mdOnOverlay = e.target === overlay; });
  overlay.addEventListener('click', e => { if (e.target === overlay && _mdOnOverlay) closeSim431(); });
  registerPanelShortcut('KeyS', {
    getButton:  () => btn,
    getOverlay: () => overlay,
    otherOverlayIds: ALL_PANEL_OVERLAY_IDS.filter(id => id !== 'sim431-overlay'),
    open:  openSim431,
    close: closeSim431,
  });
  document.addEventListener('keydown', e => {
    if (e.key === 'Escape' && overlay.classList.contains('active')) closeSim431();
  });

  document.getElementById('sim431-round').addEventListener('click', _runRound);
  document.getElementById('sim431-reset').addEventListener('click', _resetBattle);
  document.getElementById('sim431-use-potion').addEventListener('click', _usePotion);

  document.getElementById('sim431-roll').addEventListener('click', () => {
    const d = _data();
    ...
    _appendLog(d, t('battlesim431.log.rolled', { cs: d.combatSkillInitial, en: d.enduranceInitial }));
    saveState();
    _renderAll();
  });

  document.getElementById('sim431-enemy-pick').addEventListener('input', e => {
    const d = _data();
    if (!d) return;
    d.enemy.name = e.target.value;
    saveState();
  });
  _setupAutocomplete('sim431-enemy-pick', 'sim431-enemy-pick-dropdown', enemy => {
    const d = _data();
    if (!d) return;
  ...
  const fieldMap = {
    ...
    'sim431-atkmod': ['attackModifier'],
    'sim431-enemy-skill': ['enemy', 'skill'], 'sim431-enemy-en': ['enemy', 'endurance'], 'sim431-enemy-enmax': ['enemy', 'enduranceMax'],
  };
  for (const [id, path] of Object.entries(fieldMap)) {
    ...
    input.addEventListener('change', () => {
      const d = _data();
      if (!d) return;
      const allowNegative = id === 'sim431-atkmod';
      ...
      if (id === 'sim431-cs' || id === 'sim431-atkmod' || id === 'sim431-enemy-skill') d.ratio = _effectiveSkill(d) - d.enemy.skill;
      saveState();
      _renderInputs(true);
    });
  }

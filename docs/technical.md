...
Layer 3 (feature modules - import from layers 0–2 as needed):
  notes.js, battlesim829.js, battlesim8.js, battlesim286.js, battlesim198.js, battlesim199.js, auth.js, add-book.js, edit-book.js,
...
**All `.bsim-modal` battle-sim dialogs use a fixed `height: min(640px, 90vh)`, not the shared `.inv-modal`'s `max-height: 90vh`.** Internal panels (`.bsim-tech-list`, `.bsim-history-list`, `.bsim-log`) scroll within that fixed budget. Requires `min-height: 0` on `.bsim-col` (both columns) for flexbox to let those internal scroll areas shrink instead of stretching the whole modal.

**Enemy-name autocomplete fields, all five battle sims:** built as `role="combobox"` + a custom `<ul role="listbox">` (not a native `<datalist>`) to keep the app's own dropdown instead of the browser's. Two Chromium behaviors otherwise leak through regardless of `autocomplete="off"`: its form-value-history popup (suppressed with `readonly`-until-`focus`) and its contact/address-autofill heuristic, which classifies any field whose `id` contains the substring "name" as a person's name (independent of ARIA role or the lack of a `name` attribute) - so these ids are `*-enemy-pick`, never `*-enemy-name`. Applies to `battlesim198.js`/`199.js`/`286.js`/`829.js`/`8.js`.

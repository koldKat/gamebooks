...
Layer 3 (feature modules - import from layers 0–2 as needed):
  notes.js, battlesim829.js, battlesim8.js, battlesim286.js, battlesim198.js, battlesim199.js, battlesim200.js, battlesim186.js, battlesim201.js, battlesim202.js, battlesim203.js, battlesim83.js, battlesim86.js, battlesim114.js, battlesim115.js, battlesim123.js, battlesim130.js, battlesim92.js, battlesim108.js, battlesim216.js, battlesim193.js, battlesim217.js, battlesim526.js, battlesim322.js, battlesim323.js, battlesim324.js, battlesim325.js, battlesim122.js, battlesim80.js, battlesim82.js, battlesim118.js, battlesim218.js, battlesim430.js, battlesim204.js, battlesim205.js, battlesim206.js, battlesim207.js, battlesim208.js, battlesim209.js, battlesim210.js, battlesim211.js, battlesim212.js, battlesim213.js, battlesim214.js, battlesim215.js, auth.js, add-book.js, edit-book.js,
...
- `#feed-content` shows a `.feed-loading` spinner (`.feed-spinner`, plain CSS border-spin, not gated behind `reduce-motion` since it's functional) only when the panel doesn't already contain a `.feed-entry`/`#feed-header` - i.e. a genuinely first/empty load, not `livetab.js`'s own 60-second leader-tab poll re-calling `loadFeed()` in the background, which must swap fresh entries in directly without a visible spinner flash.
...
**Loading state:** `_showSection()`/`_showExtra()` both replace `#liveread-body` with a `.liveread-loading` spinner (`.liveread-spinner`, plain CSS border-spin) before fetching, then overwrite it with the real HTML once the response resolves - same visual pattern as mobile's `.m-spinner`, not gated behind `reduce-motion` since it's a functional loading indicator rather than a decorative animation. The spinner only shows for the first section fetched after the panel opens (`_isFirstShowSinceOpen`, reset in `_open()`/`_close()`) - normal page-to-page reading swaps text with no spinner flash, and a cache hit (see below) skips it even on that first show.

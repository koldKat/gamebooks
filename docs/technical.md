...
For each missing node, `_assignGridPositions()` places it one grid column past its positioned incoming parents - falling back to any positioned neighbor, then to raw `depth * _GRID_LAYER_GAP` only when no neighbor is positioned - processing missing nodes in ascending BFS-depth order so parents are positioned before their children. When `state.snapToGrid` is on, the placed x/y are rounded to `GRID_SIZE`. Mobile's `_layout()` has the equivalent logic via its own local `_positionedNeighbors()` and always rounds its placements to `GRID_SIZE` (mobile has no snap toggle - drags always snap). Every node the grid places gets `physics: false`. `_assignGridPositions` only ever fills gaps, never moves an existing position.

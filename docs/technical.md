  ...
  public/
    ...
    js/
      ...
      battlesim/         All battle simulator modules, one file per book, grouped in their own
        ...
        battlesim324.js    Battle simulator for book 324, The Chasm of Doom (Lone Wolf book 4) - reuses book 193's COMBAT_TABLE and Combat Ratio system unchanged, verbatim-identical rules text confirmed the same way as books 322/323. book_enemies (52 rows, read from all 350 sections) uses the same .attack/.hp convention; recurring Bandit Warrior/Horseman encounters at different sections are kept as separate rows disambiguated by section number, since their COMBAT SKILL/ENDURANCE genuinely vary between tellings; one same-name/same-stat/same-destination pair (Stoneworm §26=§88) is merged into one row; Barraka's two identical tellings (§122, §325) are likewise one row; Giant Meresquid appears at three separate, non-identical-destination sections (§47, §194, §234, all COMBAT SKILL 16/ENDURANCE 37) and is kept as three separate rows. This book's own imported text had 30 sections with a missing-choice-link import defect (plain "turning to N" text with no href/choices entry) and 7 sections with Project Aon footnote markers/bodies glued onto the wrong paragraph - same defect classes as books 322/323's own audits - both fixed in the same pass; its final section (§350) also had the full Project Aon backmatter (errata, footnote index, section index) dumped onto the story ending, same as book 322's §350, stripped down to the real ending.
        battlesim325.js    Battle simulator for book 325, Shadow on the Sand (Lone Wolf book 5) - reuses book 193's COMBAT_TABLE and Combat Ratio system unchanged, verbatim-identical rules text confirmed the same way as books 322/323/324. The §4 Palace Gaoler fight's special rule (ignore enemy ENDURANCE loss in round 1, destination depends on total round count) is not modeled and is handled by hand. book_enemies (38 rows, read from all 350 sections) uses the same .attack/.hp convention; one same-name/same-stat/same-destination pair (Itikar §240=§370) is merged into one row. This book is structured in two parts of 200 sections each (a unique format for this series); its own imported text had 9 sections with a missing-choice-link defect and 9 with footnote markers/bodies glued onto the wrong paragraph, same defect classes as books 322-324's audits; both the Part I/Part II split point (§200) and the true final section (§400) had the Project Aon backmatter (errata, footnote index, section index) dumped onto the story text - fixed the same way as book 322's §350, stripped down to the real story text at each.
        ...
        battlesim223.js    Battle simulator for book 223, Star Strider (Fighting Fantasy, sci-fi bounty-hunter setting) - a plain FF SKILL/STAMINA/LUCK sim, confirmed from this book's own front-matter rules text (not assumed), reusing attackModifier/enemyWoundDamage/playerWoundDamage/enemyAutoWinFirstRound/enemyDefeatThreshold unchanged from book 220's template (none are exercised by default for this book's roster - every fight here is a plain STAMINA-0 defeat). No Provisions/Potions UI - no such mechanic exists in this book's text; STAMINA recoveries are one-off narrative events applied with the stat stepper. FEAR and TIME are separate tracked resources in the book's own rules (a countdown clock and a psychological test) but neither is combat math and neither is modeled. extraAttackers (0-3) covers this book's simultaneous-enemy encounters: 2x Bandit (§157, printed as a "SPECIAL Encounter Box" vehicle-chase fight that per the book's own rules should not reduce the player's real STAMINA), 3x Thug (§248), 2x Guard (§371), 4x Guard (§343). One book-specific mechanic not covered by any generic field: the book's own Combat rules state a double-6 Attack Strength roll instantly deactivates any Android-type enemy regardless of its remaining STAMINA - not automated (nothing in the sim marks a roll as "this was a double"), watch the round log for a 12 and zero the Android's STAMINA by hand when it appears. book_enemies (39 rows, read from all 400 sections) uses the same .attack/.hp convention as every other FF sim; recurring names across different encounters (four separate GromPol Android fights, two Horned Dog-Faced Guard fights, two Rogue fights) are kept as separate rows disambiguated by section number in the name field, since their STAMINA values genuinely differ between encounters rather than being exact repeats. The Grom's (§201) "SKILL 10 CUNNING 10" stat line is not a combat encounter - CUNNING is a different, non-STAMINA-based contest this sim doesn't model - and was excluded from the roster rather than force-fit into the STAMINA field.
        battlesim224.js    Battle simulator for book 224, Phantoms of Fear (Fighting Fantasy 28 by Robin Waterfield) - a plain FF SKILL/STAMINA/LUCK sim, confirmed from this book's own "Battles" rules text. This book adds a fourth stat, POWER, for spellcasting and the dream-world mechanic - not part of the combat math, same precedent as every other spell-stat in this app's FF sims. No Provisions/Potions UI - not checked against this book's own equipment rules in depth; STAMINA recoveries are one-off narrative events, hand-applied with the stat stepper. extraAttackers (max 1) covers this book's one genuinely simultaneous encounter, the two N'Yadach at §184 (the book's own text says both attack every round and you pick which one to fight back against); every other multi-enemy fight (Orcs §14, Wolves §79/§273, Prowlers §130, Sciacalls §311, three Dark Elves §323) is fought one at a time per the book's own text. Not modeled as a bespoke mechanic: the six Dark Elves "running battle" at §336, where failing to beat one within 4 Attack Rounds lets the next one in line get free strikes - count rounds from the visible log and apply the -2 STAMINA penalty by hand. ISHTRA (§201) and MORPHEUS (§364) fight with POWER substituted for STAMINA per the book's own text - seeded in book_enemies with their POWER score in the .hp column, but the player's own STAMINA field must be swapped for their current POWER by hand before and after that fight. book_enemies (68 rows, read from all stat-block-bearing sections of 400 total) uses the same .attack/.hp convention as every other FF sim; recurring same-name/same-stat creatures across different sections (COOK, BANSHEE, SKELETON SPIRIT, WILD MAN, WEEVIL MAN, SHAPECHANGER, the Wolves) are kept as separate rows disambiguated by section number. Source: a hand-verified HTML extraction supplied directly rather than a fresh PDF import (the book's own PDF import had previously been reverted for corruption) - parsed cleanly with 0 broken links and 0 structural defects on the first pass, including a subtle dream-world/real-world paragraph-flip mechanic (subtract 20 / add 50) that the extraction had already correctly annotated per-section.
        battlesim317.js    Battle simulator for book 317, The Battlepits of Krarth (Blood Sword book 1 by Dave Morris & Oliver Johnson) - the English original of book 78's Bulgarian translation, confirmed identical section numbering between the two editions. Four attributes: Fighting Prowess (to-hit), Psychic Ability (narrative/spell-resist only, not modeled), Awareness (turn-order only, not modeled), Endurance (hp). Per-round choice of Attack (2d6 to-hit, then 1-2 damage dice + flat bonus minus target Armour Rating) or Defend (skips own attack, forces the enemy to roll 3d6 instead of 2d6 to hit). Rank VIII solo-Adventurer starting stats read from this book's own class tables, matching book 78 exactly: Warrior FP 9/Dmg 3d6+1/End 48/Armour 3, Trickster FP 8/Dmg 3d6/End 48/Armour 2, Sage FP 8/Dmg 3d6/End 40/Armour 2, Enchanter FP 7/Dmg 2d6+2/End 40/Armour 2. book_enemies (31 rows, read from all 51 stat-block-bearing sections of 534 total) uses attack=Fighting Prowess, hp=Endurance, pb=damage dice count, defense=damage flat bonus; Armour Rating has no column and always resets to 0 on enemy pick. Recurring encounters (Assassins, Barbarians, three Skiapyr power tiers, Magus Vyl, Icon the Ungodly, Giant Spiders, Adventurers, Ranger) are seeded once at their representative value; Skrymir the Giant is fought three times at genuinely different stats (§133 End 70, §342 End 55, §539 End 55/Dmg 5d6+6) and kept as three separate notes rather than merged. Imragarn (§124) is excluded from the roster - a resurrected NPC who joins the player's side, not an enemy.
        battlesim318.js    Battle simulator for book 318, The Kingdom of Wyrd (Blood Sword book 2 by Dave Morris & Oliver Johnson) - the English original of book 135's Bulgarian translation, confirmed identical section numbering between the two editions. Same mechanic as battlesim317.js. Rank XII solo-Adventurer starting stats read from this book's own class tables, matching book 135 exactly: Warrior FP 9/Dmg 4d6+2/End 72/Armour 3, Trickster FP 8/Dmg 4d6+1/End 72/Armour 2, Sage FP 8/Dmg 4d6+1/End 60/Armour 2, Enchanter FP 8/Dmg 3d6+2/End 60/Armour 2. book_enemies (30 rows, read from all 51 stat-block-bearing sections of 569 total) uses the same column convention as battlesim317.js. Recurring encounters (Zombie, Snow Vampires, Frost Hounds, Thanatos the Giant, Chieftain, Sylphs, Werewolves + Servants, Stalker, Skeleton Guards, Jadhak, Thanes) are seeded once at their representative value; the Stalker's Endurance genuinely varies by encounter (regenerates by magic between fights per the source text) and is called out for hand-adjustment per specific section. The Lady in Grey (§421) is excluded - a spell-only target with no printed Fighting Prowess, can't be melee-attacked.
        battlesim319.js    Battle simulator for book 319, The Demon's Claw (Blood Sword book 3 by Dave Morris & Oliver Johnson) - the English original of book 107's Bulgarian translation, confirmed identical section numbering between the two editions. Same mechanic as battlesim317.js/battlesim318.js. Rank-16 solo-Adventurer starting stats read fresh from this book's own class tables: Warrior FP 10/Dmg 6d6/End 96/Armour 3, Trickster FP 9/Dmg 5d6+2/End 96/Armour 2, Sage FP 9/Dmg 5d6+2/End 80/Armour 2, Enchanter FP 9/Dmg 4d6+2/End 80/Armour 2 - the Warrior figure corrects book 107's Bulgarian value (FP 10/Dmg 5d6+1), which had been flagged there as an OCR-garbled reconstruction; this book's English text is unambiguous. book_enemies (24 rows, read from all stat-block-bearing sections of 588 total) uses the same column convention as battlesim317.js. The 3-demon-god boss trio (Azidahaka/Nasu/The Yazir, §300/§330/§410/§457) is kept as 3 separate rows; Azidahaka's damage (12d6) resolves a data gap flagged in book 107, where the same line printed as a bare "12" with no dice-count word - this book's English text states "12 Dice" outright. The Seven-in-One (7-stage boss, §87/§204/§240/§243/§361/§382/§536) is seeded at its first form only, with the other 6 forms' stats noted in-file for hand-adjustment.
        battlesim320.js    Battle simulator for book 320, Doomwalk (Blood Sword book 4 by Dave Morris & Oliver Johnson) - no Bulgarian translation of this book exists in this app's DB, so the roster was read fresh from this book's own English text with no sibling sim to cross-check against. Same mechanic as battlesim317.js-battlesim319.js. Rank-16 solo-Adventurer starting stats match book 319's exactly (confirmed independently, not assumed): Warrior FP 10/Dmg 6d6/End 96/Armour 3, Trickster FP 9/Dmg 5d6+2/End 96/Armour 2, Sage FP 9/Dmg 5d6+2/End 80/Armour 2, Enchanter FP 9/Dmg 4d6+2/End 80/Armour 2. book_enemies (28 rows, read from all stat-block-bearing sections of 557 total) uses the same column convention as battlesim317.js. Angvar (§54/§394) is printed with Endurance 950 in both tellings - included as-is, flagged in-file as an effectively-unwinnable narrative encounter rather than a realistic sim target. Icon-simulacra (§240) has no printed Endurance at all per the source text itself ("You won't know their Endurance scores until you wound them") - seeded at a placeholder value matching the real Icon's own stats, flagged as unconfirmed.
        battlesim321.js    Battle simulator for book 321, The Walls of Spyte (Blood Sword book 5 by Dave Morris & Oliver Johnson, the series finale) - no Bulgarian translation of this book exists in this app's DB. Same mechanic as battlesim317.js-battlesim320.js. Solo-Adventurer rank is 20th here (higher than books 317-320's 16th, per this book's own intro text), with rank-20 stats read fresh from this book's own class tables: Warrior FP 11/Dmg 7d6+1/End 120/Armour 4 (plate armour, an upgrade from the earlier books' chainmail Armour 3), Trickster FP 10/Dmg 7d6/End 120/Armour 2, Sage FP 10/Dmg 7d6/End 100/Armour 2, Enchanter FP 9/Dmg 5d6+2/End 100/Armour 2. book_enemies (31 rows, read from all stat-block-bearing sections of 550 total) uses the same column convention as battlesim317.js. This book's imported text required an extensive corruption-repair pass before the roster could be extracted (~250+ sections had page-footer-digit collisions splicing unrelated content together); see git history for that cleanup. The True Magi (§347/§452, the final-boss quintet) is seeded at §452's later/stronger telling (Endurance 75 each vs §347's 55) rather than merged; both instances grow further by +1 Psychic Ability/+10 Endurance per Round, not modeled. Snorrid the Giant (§385, Endurance 950) is an effectively-unwinnable narrative encounter, included for completeness only. Several per-encounter special abilities (Onaka's acid-spit/spell alternate attacks, the Demon-Lord's five-strikes-per-Round, the Argus's strike-up-to-four-adjacent-players, the Kraken's attack-the-barge option, the Kraken/Dragon's non-melee special attacks) aren't modeled - this sim represents each enemy's plain melee option only.

        battlesim370.js    Battle simulator for book 370, Узурпатор! (The Usurper, Way of the Tiger 1) - not a Fighting Fantasy SKILL/STAMINA/LUCK sim; this book's own rules text ("ПРАВИЛА НА ИГРАТА"/"СРАЖЕНИЯ") uses three independent technique scores (удар с ръка/Hand, удар с крак/Kick, хвърляне/Throw, each starting at 0 and boosted once by an on-paper 1d6 chargen roll, free-entry rather than re-rolled here) plus a separate 5-count Shuriken resource with no score of its own. To-hit is 2d6 + the chosen technique's score against a per-encounter, per-technique enemy Defense value (the same named enemy is fought via different techniques on different branches, each with its own stated Defense, so all four Defense fields are free-entry per fight rather than looked up automatically beyond an initial autocomplete fill). On a Hand/Kick/Shuriken hit, damage is a re-rolled 1d6 added to the raw 2d6 to-hit roll (Kick adds a further flat +2); Throw deals no direct damage on a hit but grants an immediate free follow-up Hand or Kick attack (that attack's own Defense field, +2 damage on top of Kick's own +2 if it lands). Inner Force (starts at 5) may be spent before any roll to double the damage sum if the attack lands, lost regardless of outcome. Block, offered only after being hit, avoids the hit entirely on a 2d6 roll under the player's per-battle Defense; whether it succeeds or fails, the next attack's technique score takes a one-shot -2 penalty. Съдба (Fate) is explicitly narrative-only per the book's own text and isn't modeled. The rules state that a failed Throw reduces the player's defense for the enemy's counterattack but never give a number anywhere in the text or its two worked examples, so that reduction is left unquantified - the counterattack uses the player's normal per-battle Defense field. book_enemies (12 rows, read from ~30 in-story encounters across the rules' two worked examples and the full story text) reuses its four numeric columns non-standardly for this book's shape: .hp holds Издръжливост (confirmed identical across every branch of a given named enemy) EXCEPT weakened Астарот (not the full-strength Седмият херцог на ада Астарот), whose Endurance is 2 on one branch (§189) and 6 on another (§388/§412) despite narrating the same weakened-form fight - re-enter per encounter for this one enemy; .attack holds a representative starting Hand-strike Defense (an autocomplete-fill default only, re-entered per encounter), and .pb/.defense together reconstruct the enemy's damage formula as dice-count/flat-bonus. Призрак Бандит (§307/§313) never prints a damage line in the source text for either encounter (a miss sends the player to a narrative soul-drain section instead of a numeric loss) - confirmed via source re-check, seeded at 1 die + 0 purely as autocomplete filler; treat as pass/fail, not counter-damage. Четирима полуоркове (§187, 4-body group fight) uses a flat fumble check per attack rather than an opposed roll and is not modeled by the sim; its row exists only for manual autocomplete.
        battlesim375.js    Battle simulator for book 375, Убиец! (The Assassin, Way of the Tiger 2) - same combat system as battlesim370.js; this book DOES reprint its own full "СРАЖЕНИЯ" rules recap (identical mechanics to book 370's, plus a short "СПЕЦИАЛНИ ПРАВИЛА ЗА КНИГА 2" carry-over-character addendum) - captured verbatim in book_frontmatter.rules_text - confirmed identical from both the rules text and the in-story stat-block format itself ("[name] Защита срещу [техника] «[име]»: N Издръжливост: N Щети: N зар[+N]"). book_enemies (13 rows, read from all 25 in-story stat blocks): Първо/Второ бойно псе's Endurance is genuinely inconsistent across its three re-tellings (§268 gives 14/12, §284 gives 14/16, §327 gives a single dog at 12) despite all three branches leading to the same §353 - seeded at §268's 14/12 rather than guessing which telling is authoritative. Древен бог similarly shows 22 (§321) vs 26 (§333) across its two branches; seeded at 22. Хання, the story's antagonist, is deliberately NOT seeded - she has no Endurance/Defense stat block anywhere in the text and is defeated via a scripted Luck-gated mind-control check (§293/§390) followed by one specific dagger or Inner-Force-punch attack (§247/§298), not round-based combat. Крал на таласъмите's companion "Танцуващ меч" has a stated damage line but "-" for Defense/Endurance (can't be targeted directly); folded into the King's own encounter as a second passive attack rather than seeded as its own row.
        battlesim376.js    Battle simulator for book 376, Властелин! (The Overlord, Way of the Tiger 3) - same combat system as battlesim370.js/battlesim375.js, confirmed from this book's own ~20 in-story stat blocks; this book DOES reprint its own "ПРАВИЛА НА ИГРАТА ЗА КНИГА ВТОРА" rules recap (same combat mechanics as book 370, captured verbatim in book_frontmatter.rules_text). book_enemies (10 rows): Първи/Втори телохранител is a simultaneous two-enemy fight (same shape as battlesim375.js's Battle Dogs) - player Defense drops to 6 while both are alive (blocking only one attack per round) and rises to 8 once one falls. "Рогат циклоп" appears as two distinct encounters under the same displayed name with different damage formulas and a different-named Kick move (§250/262 → §298, 2 dice damage; §322/334 → §70, 1 die+3 damage) - kept as two separate book_enemies rows ("Рогат циклоп" and "Рогат циклоп (втора среща)") rather than merged, since they lead to different destinations and aren't retellings of the same fight. Жрец на Немезида's Throw option (§128) is a scripted magical interrupt that always fails for a flat 2-damage cost, not a normal Defense-gated roll - left unmodeled (Def Throw=0, unused field) rather than forced into the roll-based shape. Нинджа с кусаригама's Throw option (§280) is a scripted instant win with no roll at all - same reasoning, unmodeled.
        battlesim377.js    Battle simulator for book 377, Завоевател! (The Conqueror, Way of the Tiger 4) - same combat system as battlesim370.js/battlesim375.js/battlesim376.js, confirmed from this book's own 17 in-story stat blocks; this book DOES reprint its own "ПРАВИЛА НА ИГРАТА ЗА КНИГА ТРЕТА" rules recap (same combat mechanics, captured verbatim in book_frontmatter.rules_text). One book-specific addition, a "нервнопаралитичен удар" (paralytic-strike) skill on the Хонорик fight adding a flat +2 damage on hit, explicitly mutually exclusive with Inner Force - not modeled as a separate toggle, since Inner Force's damage-doubling is strictly better whenever both are available, so a player holding both skills would never pick the weaker one; the sim's existing manual enemy-dmgBonus field can represent it by hand for that one fight if needed. book_enemies (5 rows): Пещерен трол appears as two distinct encounters under the same displayed name (§173/195/211 → §19; §337/347/357 → §411) kept as separate rows, same reasoning as book 376's двата Рогат циклоп. Both Пещерен трол rows and Изчадие на процепа reuse a single defKick field for two differently-named Kick moves each (e.g. «тигров скок» vs «вършачка на Куон») that share the same UI field - populated with the more frequently offered branch's value, the other branch's slightly different Defense must be hand-adjusted if simulating that specific branch. Старец's alternate "Скиптър" weapon option is folded into the Hand Defense field the same way, since the two options never differ in outcome shape. Изчадие на процепа's fight rules forbid Block entirely ("Не можеш да използваш блок") - not hard-enforced by the UI, a player simulating this fight should choose Take Hit instead of Block each time. Мардолх (§223) is excluded from the roster - an infinite-retry single opposed-roll check with no Mardolh Endurance value printed at all (unlike book 375's Мардолх, which has a full 30-Endurance stat block - the two are not the same encounter shape despite the shared name).
        battlesim378.js    Battle simulator for book 378, Пъкъл! (Hell!, Way of the Tiger 5, final book) - same combat system as battlesim370.js/battlesim375.js/battlesim376.js/battlesim377.js, confirmed from this book's own 20 stat-block-bearing sections; this book DOES reprint its own book-specific rules recap (same combat mechanics, captured verbatim in book_frontmatter.rules_text); no new climactic mechanic found. book_enemies (6 rows): Лорд Сайл, Вожд на Орковете (End 18, Dmg 1d+2, Def Kick=5/Hand=7). Кочияш орк/Втори орк/Трети орк - three orcs fought simultaneously as one scene (§162, same shape as book 376's paired bodyguards), kept as three separate rows since each has genuinely different stats (End 10/8/7, Def 6/5/5, Dmg 1d+1/1d/1d); the player's own combined Defense against all three (8) isn't stored in book_enemies, same as every other multi-enemy fight in this series - set manually per fight. Касандра (End 18, Dmg 1d+3 - source printed "1 зар + З", the "З" a Cyrillic/Latin-lookalike OCR misread of "3", corrected here) has Defense given only for Hand («ухапване на кобра») and Throw («водовъртеж»), both 8; a third Kick branch is offered in-story but never gets its own printed Defense value, so defKick is left at the field default rather than guessed. Тютчев (End 20, Dmg 2d+2) is fought across three narrative retellings of the same "three battles in a row" climactic duel (same shape as book 375's recurring Старец) with a single generic "Защита срещу Пътя на тигъра" Defense that applies to any of the three techniques equally - seeded into defHand only, since book_enemies has no all-technique column; defKick/defThrow must be set to match by hand if simulating those techniques specifically. Two of the three retellings state Defense 8, the third states 7 (with the player's own counter-defense listed as 9 instead of 8) - a minor source inconsistency, seeded at the majority value. Excluded from the roster: a recurring silver serpent-headed spear dodge/block sequence and every generic "Джудже-трол" (dwarf-troll) axe-fighter skirmish, neither of which ever prints an Endurance value - single opposed-roll checks, not HP-tracked fights, same judgment as book 377's Мардолх.

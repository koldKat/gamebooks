...
**Coverless day cards are genuinely transparent, not a synced copy:** `.feed-day-card--glass` (and, with "Show covers in feed" off, every `.feed-day-card`) is just `background-color: transparent` with a `rgba(31,41,55,0.9)` `::after` overlay for legibility - nothing else. Neither `#feed-panel`, `#landing-wrapper`, nor `body` set an opaque background, so this lets the real, fixed `#landing-bg-a`/`-b` layers (z-index behind everything) show straight through, same pixels every frame, crossfades included automatically. An earlier version faked this with a copied `background-image` kept in sync via a `--landing-cover-url` custom property (plus a second `::before` layer to crossfade between old/new copies) - removed 2026-08-09 after it produced two real bugs: the copy visibly lagged the real background's own 1.5s crossfade (since `background-image` can't be transitioned, and the copy could only ever be "as fresh as the last JS update"), and a stale copy could briefly reappear after the feed panel was hidden and restored (collapsing to `max-height: 0` apparently let the browser skip the pending opacity transition). True transparency has nothing to keep in sync, so neither failure mode is possible any more.

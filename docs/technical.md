...
**UI:** a floating panel (`liveread-panel`, `public/css/liveread.css`), docked right, internally scrollable, toggled by an explicit button in `#play-btn-row`. Deliberately not built on the `.inv-overlay` blocking-modal pattern used by every other panel in the app - it has no full-screen backdrop, so the graph stays visible and interactive underneath while reading.

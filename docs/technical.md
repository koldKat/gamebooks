...
Layer 3 (feature modules - import from layers 0–2 as needed):
  notes.js, battlesim829.js, battlesim8.js, battlesim286.js, battlesim198.js, battlesim199.js, battlesim200.js, battlesim186.js, auth.js, add-book.js, edit-book.js,
...
**`esc()`** (`server/forum.js`) escapes `& < > " '` - used inline in `onclick="location.href='...'"` attributes (category cards/breadcrumbs), so the quote-escaping matters there specifically.

Thread titles render as `<a href="...">`, not `<button onclick="...">` - buttons are atomic inline boxes that don't participate in text truncation, so a title that needed `text-overflow: ellipsis` inside a button rendered as nothing but the ellipsis instead of a partially-clipped title.

`renderBody()` in `server/forum.js` supports `[label](url)` links, `\n`→`<br>`, and the same `**bold**`/`*italic*`/`__underline__`/`~~strikethrough~~`/`{color:name}...{/color}` regex chain as the announcement system's `formatAnnBody()` (`feed.js`, duplicated in `admin/js/announcements.js`; see `FORUM_COLORS`).

**Forum body markup rendering exists in two places:** `renderBody()` server-side (initial page load) and `_renderBody()` client-side in `forum.js`'s embedded script (in-place re-render after editing a thread/post, so a save doesn't require a full reload). The two must be kept in sync by hand - `_renderBody()` is a regex-chain mirror of `renderBody()`, not a shared function. Since that client script is itself emitted from a server-side template literal, every backslash in its regexes must be doubled to survive the round-trip.

**Editing a thread title updates three places together:** the `#thread-title-display` heading, the breadcrumb's title segment (`#breadcrumb-title`), and `document.title` - all three must be touched by the edit handler or the breadcrumb/tab title go stale until a reload.

Announcements and forum posts can link to a book (`[Label](/book/:id)`) and open its in-app detail dialog instead of navigating away: a click-interceptor (`feed.js` for announcements, `shell()` in `server/forum.js` for forum posts) catches same-origin `/book/(\d+)` links and calls `openCoverActivity(bookId, ...)`/the `gamebooks-open-book` postMessage bridge. **Book links must always be relative** (`/book/:id`), never an absolute hardcoded domain - the app is served from multiple domains (koldkat.net, pathmap.net, bookplay.net), and a baked-in absolute URL resolves to the wrong domain everywhere else. `renderBody()`, `_renderBody()`, `formatAnnBody()` (`feed.js`/`admin/js/announcements.js`), and the no-JS SEO page's `fmtAnnBody()` (`server/routes/public.js`) all accept the relative form and render it without `target="_blank"`; genuine external `https://` links keep `target="_blank"`. The admin composer preview is the one exception - it always uses `target="_blank"` regardless of relative/absolute, since the admin panel has no in-app place to send the click.

Link colors: `.feed-ann-body a`/`.feed-pinned-body a` (`demo.css`) and `.ann-card-body a` (`admin/index.html`) are themed to the app's `#60a5fa` accent; `server/forum.js`'s `.post-body` and the no-JS SEO page's `.ann-body`/`.pinned-body` already had a global `a { color: #60a5fa; }` rule and only needed `a:visited` added.

**Forum modal z-index and lifecycle:** `#forum-modal-overlay` sits at `z-index: 3000`, above `#public-modal-overlay`'s (`.pub-overlay`) baseline `300` and `#edit-book-modal-overlay`'s `2000`. Opening a book/series detail dialog from inside the forum bumps `#public-modal-overlay` to an inline `z-index: 3001` (reset to the CSS default in `closePublicModal()`) rather than closing the forum, so the dialog renders above it instead of behind it. `showBooks()`, `navigateToBook()`, and `openEditBookModal()` (`boot.js`/`edit-book.js`) each unconditionally close the forum modal as their first action, since all three represent "leaving the quick-look dialog for something real" and are each reachable from multiple call sites. `openForumModal()` always resets the iframe to `/forum` home on reopen via `forumFrame.contentWindow.location.replace(url)` (comparing the `<iframe>`'s `src` attribute doesn't work, since in-iframe navigation never touches it).

**Demo mode has no real account or token** - any authenticated `apiFetch()` call made from a demo-reachable code path 401s, and `apiFetch`'s global 401 handler unconditionally logs the session out (`clearToken()`/`clearUsername()` + `auth-expired` → `showLogin()`), which happens silently behind whatever modal triggered it. Every dialog reachable in demo mode (Create Book/Anthology/Series, Edit Book including its rating widget) guards its authenticated calls with `isDemoMode` and shows `addbook.demo_not_supported` instead of firing the request. `books.js`'s `isCreator` check treats demo mode as the creator of its own fake book, so its Edit button is reachable and needs the same guards.
...
**Forum modal uses a fixed `height: 92vh`, not shrink-to-fit** - same-origin iframes don't auto-size to content, so shrink-to-fit would need cross-frame `postMessage`/`ResizeObserver` plumbing; the modal reserves a fixed height instead.
...
**Social preview image:** `og:image`/`twitter:image` point to a pre-rendered `og-image.png` (1200x630, generated via `rsvg-convert -w 1200 -h 630 og-image.svg -o og-image.png`, not regenerated at request time) - most link-preview platforms don't reliably rasterize SVG. Referenced in `index.html`, `guide.html`, and the SSR meta-injection points in `server.js` (book/anthology pages, public profile pages) that fall back to the site-wide image when a book/user has no cover/avatar.
...
**`book_enemies`** - reference enemy stat blocks feeding the enemy-name autocomplete inside a book's battle simulator (`GET /api/books/:id/enemies`). No admin UI exists for this table yet - rows are seeded by hand via direct SQL against `database.sqlite`, extracted from each book's actual combat encounters. `attack`/`defense`/`pb` map to book 829's opposed Attack/Defense/Proектоброня system (`battlesim829.js`); books using a different combat model (e.g. book 286's flat weapon min-hit system) repurpose `attack` to mean whatever that book's own simulator needs it to (for 286, "enemy minimum hit"; for book 198's standard Fighting Fantasy SKILL/STAMINA/LUCK system, `attack`=SKILL and `hp`=STAMINA - books 199 and 200 use the same convention), leaving `defense`/`pb` at 0. Book 186 goes a step further and repurposes `attack`/`hp` two different ways *within the same table*, depending on which of its three combat systems a given row is for: SKILL/STAMINA for hand-to-hand and phaser rows (`hp` left `NULL` for phaser-only rows, which don't track STAMINA), WEAPONS STRENGTH/SHIELDS for ship-to-ship rows - `battlesim186.js` has two separate autocomplete pickers pointed at the same table, each reading the pair according to whichever mode it's in. Book 8 doesn't use named per-section enemies at all - its rows are generic Skill/Life variant templates for `battlesim8.js`'s own dice-based enemy generator, not real book encounters.

**`battlesim199.js` MAGIC/spell system:** MAGIC score (2d6+6) is a total budget of spell-casts, freely allocated by the player across the 12 spells via the "have" stepper on each (matches the book's own "USING MAGIC" rule verbatim - choose spells up front, cross one off your list each time you cast it). Allocation is editable at any time, pre- or post-roll, with no enforced cap against the MAGIC total - the sim trusts the player the same way it trusts every other stat field. Skill/Stamina/Luck Spells auto-restore that stat by `floor(Initial/2)` capped at Initial; the other 9 are narrative-only (logged, decremented, effect applied by hand). Sun-Sword (+4 SKILL) and the Balthus Dire endgame's enchanted sword (+2 SKILL-equivalent) are toggleable fixed-bonus items applied via `_effectiveSkill()`; Potion of Magik is a 2-dose consumable permanently raising MAGIC by 1/dose. No Provisions mechanic in this book (STAMINA only restored via the Stamina Spell).

**`.bsim-tech-row--depleted` styling convention:** dims a row to `opacity: 0.45` to mark it as currently unusable - only apply it to the specific controls that are actually inactive (e.g. a spell's "left" count and Cast button once exhausted, or a consumable's Drink button before its "Have it" checkbox is checked), never to a wrapper that also contains an always-usable control like a stepper or checkbox, or the always-usable control ends up looking disabled too even though it still works.

**`battlesim199.js` MAGIC is a single shared pool, not per-spell:** casting any spell decrements `d.player.magic` by 1 (in addition to that spell's own `used` counter) and is blocked once `d.player.magic` reaches 0, regardless of individual spells' remaining `have`. The per-spell `have` allocation only caps how many casts of *that* spell can be pulled from the shared pool - it isn't a separate budget per spell.

**All `.bsim-modal` battle-sim dialogs use a fixed `height: min(760px, 90vh)`, not the shared `.inv-modal`'s `max-height: 90vh`.** Internal panels (`.bsim-tech-list`, `.bsim-history-list`, `.bsim-log`) scroll within that fixed budget. Requires `min-height: 0` on `.bsim-col` (both columns) for flexbox to let those internal scroll areas shrink instead of stretching the whole modal.

**`battlesim200.js`'s Luck-event queue, and paired-attacker fights:** unlike the other battle sims' single `pendingLuck` slot, book 200 tracks `pendingLuckQueue` (an array) because one Attack Round here can land more than one Luck-eligible hit at once - the main enemy, a second "paired" attacker, and the Fire Demon's bonus whip attack can each wound the player in the same round. Test Luck/Skip processes one queued entry at a time until empty. Paired fights (Bandits, Hunting Dogs) model a second enemy (`sideEnemy`) fought via a second, independent exchange each round - a fresh player Attack Strength roll (not a reuse of the one thrown against the chosen target) compared against the side enemy's own roll; it can wound the player but the player can never wound it back, a win against it only fends off its blow. Sequential multi-wave encounters (Killer Bees, Orcs, Wild Hill Men, Vampire Bats, Death Hawks, Hobgoblins) need no special code at all - each wave is just its own named row in `book_enemies`, picked in turn from the same autocomplete used for unrelated fights.
...
**Centering, relative to its actual neighbors, not the viewport:** `#play-bottom-stack` centers between `#dice-roller-wrap` (left) and `#play-btn-row` (right, charsheet/inventory/equipment/battlesim toggle buttons), tracking both neighbors' real widths via `ResizeObserver` into CSS custom properties - `--dice-roller-w` (`dice.js`) and `--play-btn-row-w` (alongside the pre-existing `--play-btn-row-h`, in `charsheet.js`'s `getPlayBtnRow()`) - then computing the midpoint between the dice roller's right edge and the button row's left edge: `calc((var(--sidebar-w) + 12px + var(--dice-roller-w, 0px) + 100vw - 1rem - var(--play-btn-row-w, 0px)) / 2)` (a `body.sidebar-collapsed` variant drops the `var(--sidebar-w) + 12px` term to plain `12px`, matching `#dice-roller-wrap`'s own collapsed-sidebar override). Both custom properties default to `0px` until their first `ResizeObserver` callback fires.
...
**Animated XP gain (`profile.js`):** the books-screen and play-area bars/counters (not the profile modal, which always snaps instantly) tween whenever `renderBooksXpSummary(data, { fromXp })` is called with a known previous XP value. Duration scales with the user's level: `level * XP_ANIM_MS_PER_LEVEL`; level 0 renders instantly. `_animateXpTo` interpolates linearly from `_displayedXp` to the target, recomputing `_xpLevelBounds` every frame - a level-up crossed mid-animation shows the bar filling to 100%, resetting, and continuing in the new level with no special-casing. A generation counter (`_xpAnimGen`) invalidates the in-flight `requestAnimationFrame` loop when a new update arrives rather than queuing. `renderBooksXpSummary` is fed `fromXp` by `rewards.js › _processRewardSnapshot`; calls with no `fromXp` are instant (used for session-initial loads). The boost-line figure tweens in lockstep with the main counter via a parallel `_displayedBoostXp`. The profile modal's own boost line (`renderXpBlock`) is always instant.

**XP boost color:** the boost percentage text (`+X% boost`) renders in aqua (`#22d3ee`) via inline `<span>` inside `innerHTML`; the following `(Y XP)` text stays in the default color. Implemented in `_xpBoostHtml`, called from `_renderBoostAmount` (books/play, animated) and `renderXpBlock` (profile modal, instant) in `profile.js`.
...
**Staggered refresh:** `refreshAppXp()` is wrapped in `_refreshAppXpStaggered()` (`APP_XP_STAGGER_MS = 1200`) rather than invoked directly alongside `loadFeed()`, so the viewer's own personal XP bar tween and the App/Avg-Level bars' tween don't start from the same instant. `_refreshAppXpStaggered()` is also called from the app-xp SSE stream's `onmessage` (leader tab direct, follower tabs via `_broadcastLiveEvent('app_xp_event', ...)`), debounced via a single tracked timer since the stream can fire multiple times in quick succession.

**Live "someone else earned XP/GC" floaters:** admin-only, rendered while the Books/landing screen is visible (`_isBooksScreenVisible()` in `app-xp.js`). Backed by `GET /api/app-xp/stream` (token via `?token=` query; 403 unless `db.isUserAdmin`). `db.setAppXpHook(fn)` fires from inside `_awardXpTx`/`_awardCoinsTx` on every successful award with `{ userId, xpDelta }`/`{ userId, coinDelta }` - distinct from `_xpFeedHook`, which only fires on level-up and carries no amount. The hook short-circuits if `_appXpClients.size === 0`, otherwise skips the admin's own gains and pushes `{ username, xpDelta, coinDelta }` to every connected admin client.

Client side, `livetab.js` owns the connection lifecycle exactly like `_connectUserBadgeSSE`/`_disconnectUserBadgeSSE` (leader-tab-only `EventSource`). `boot.js` calls `_connectAppXpSSE()`/`_disconnectAppXpSSE()` directly right after `_isAdmin`/logout resolve, for a prompt connect the moment admin status is known - `_connectLeaderLiveServices()`/`_disconnectLeaderLiveServices()` (the functions that run on every *leader-election* transition, not just login/logout) also include the app-xp SSE alongside Feed/PublicCatalog/UserBadge, so a tab losing/gaining leadership connects/disconnects it correctly too; both no-op safely for non-admin/logged-out tabs via their own internal guards. Messages are handed to `_hooks.onAppXpEvent` and also fanned out to follower tabs via `_broadcastLiveEvent('app_xp_event', payload)` → `_applyFollowerLiveEvent`. `app-xp.js`'s `handleAppXpEvent(payload)` is the single hook implementation for both the leader and follower paths - it re-checks `getIsAdmin()` and screen visibility itself, then queues a floater (amount first, username last: `+N XP <username>`) into `#app-reward-float-layer` via the same `chip` + `setTimeout(5000)` drain pattern as `rewards.js`'s personal floater queue, just with its own independent queue/state. Horizontal position is recomputed per floater (`_positionAppRewardLayer`) as the midpoint between `#covers-panel` and `#feed-panel` - deliberately **not** the same gap `rewards.js`'s `_positionRewardLayer` centers in (that one uses `#feed-panel`→`#landing-right`, i.e. feed-to-books - a different pair of panels entirely). Colors: violet for XP gains, pink for coin gains (`.app-reward-float`/`.app-reward-float--coins`), versus the personal floaters' cyan/gold - reuses the same `@keyframes reward-float-rise` rise-and-fade animation.
...
This widget is the average of each user's own already-computed level (`db.getAppXpSummary()`'s `avgLevel` field), not "level of the average XP" (that's the App bar's `level`, a single pooled-XP figure skewed upward by a few high-XP users):
...
**`undoRun()` walking past auto-nav sections:** pops the current node, then keeps popping past forced (single-choice) nodes until landing on a real decision point (`choices.length !== 1`) or the run's start. A single-choice node isn't necessarily a *forced* passthrough though - it could be a real decision the user just recorded, so it's worth stopping at one that carries metadata (`note`/`priority`/`battle`/`color`/`portals`/`showNote`) rather than silently skipping past it - but only when that stop wouldn't immediately re-trigger auto-nav (`render()`'s `!pt.path.includes(node.choices[0])` condition, recomputed as `wouldAutoNav` during the walk-back): otherwise the popped node's own auto-nav would replay the undo's forward step right back to where it started. `undoRun()`'s stopping logic is coupled to `render()`'s auto-nav condition - if that condition's shape changes, this needs to change with it.

**Alternate start ("⚑" button):** next to New Run, hidden for open-world/series books (`_owIsOpenWorld`) since those must go through `_onNewSeriesRun` instead. Opens `#alt-start-modal-overlay` (wired in `boot.js`, mirroring the "Edit Start Node" modal's `.dice-count-row`/`±` stepper pattern, own copy of the validation logic since this doesn't rename anything, it just picks one run's entry section) via `setAltStartHandler(fn)`/`_altStartHandler`, defaulting to `state.startSection`. On save, calls `startPlaythrough(secId)` directly, bypassing `state.startSection` for that one run - supports gamebooks with a genuine second beginning (e.g. flip/dos-a-dos print editions) without needing two book entries.

**New Run picker (2+ known starts):** `_knownStartSections()` (`play.js`) builds the set of "known" starts as the book's default `state.startSection` plus every distinct `pt.path[0]` seen across `state.playthroughs`, filtered to sections that still exist in `state.graph`. With fewer than 2 known starts, "New Run" starts immediately. With 2+, `showStartPicker()` opens a dialog (`#start-picker-overlay`, reusing `showFastTravelDialog()`'s `.ft-dialog-*` classes) listing each known start, the book's default tagged `(default)`. Picking one calls `startPlaythrough(secId)` directly. A "⚑ Type a section" button falls through to the existing alt-start modal (`_altStartHandler`) for a section not yet used.

**Public run viewer and alternate starts:** `getPublicRun`/`getPublicSeriesRun` in `server/db.js` send a `startSection` field that `public-profile.js`'s `_buildPubSegNetwork` uses purely for the cosmetic yellow "start" node marker (`isStart`, has no effect on layout/traversal - the full path/graph render regardless). This is that specific run's own `pt.path[0]`, not the book's configured default start, so the public view always highlights wherever *that run* actually began (relevant for a run started via the "⚑" alternate-start button).

**Node-color logic exists in three independent reimplementations** of the same "color this node" rules - `graph.js`'s `nodeColor()` (canonical), `public-profile.js`'s `_buildPubSegNetwork`, and `server/export.js`'s `_nodeColor()`. Canonical precedence: compute fill/border from visited/mapped/end-node/etc. state first, then apply the battle-flag border-override (orange) unconditionally last, excluded only for start/current-position/final-node. All three must stay in sync with this precedence or a node's battle indicator can be silently lost under other node states.

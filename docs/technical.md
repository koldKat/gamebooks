...
**Feed scroll position across collapse/restore (added 2026-08-09):** `#feed-panel` collapses to `max-height: 0` (`body.feed-collapsed`) rather than being unmounted, which shrinks `#landing-wrapper`'s (the actual scroll container) scrollable content and lets the browser clamp its `scrollTop` - on a long feed that clamp can land well above where the user had actually scrolled to, and nothing restored it on the way back out. `_setLandingPanelCollapsed()` (`prefs.js`) now captures `#landing-wrapper.scrollTop` into a module-level `_feedScrollBeforeCollapse` right before collapsing (only when transitioning from expanded, not on redundant calls), and restores it a double-`requestAnimationFrame` after un-collapsing. Since `max-height` can't be transitioned to/from its default `none` (not an animatable length), the panel is already back to full height the instant `body.feed-collapsed` comes off - only the opacity/transform entrance animates - so the restore doesn't need to wait out a transition, just one layout flush. Shared by both the individual feed-toggle button and the `Ctrl+X` group toggle, since both go through this same function.
...
This treatment is applied by two separate selectors sharing the same declarations, at every screen width (including mobile, as of 2026-08-06 - `background-attachment: fixed` has a known history of being janky on mobile Safari, so if it turns out to actually misbehave there, reintroduce a `@media (min-width: 769px)` wrapper around both rules and let `.feed-day-card--glass`'s plain `background-color` fallback, above, cover mobile again on its own). With day covers showing (`body:not(.no-feed-day-covers)`), only `.feed-day-card--glass` (originally cover-less days) gets it - `.feed-day-card--glass` shows the same currently-live rotating cover as its own `background-image` (`background-attachment: fixed`, so it acts as a window onto the same fixed page background - always in sync, never a stale copy) with the *same* `rgba(31,41,55,0.9)` overlay a real cover-tile card gets, via its own `::after`, so the two card kinds read identically instead of one looking washed out next to the other. With "Show covers in feed" off (`body.no-feed-day-covers:not(.no-feed-glass-cards)`), *every* `.feed-day-card` gets it instead, not just originally-`--glass` ones - with covers off, none of the cards show a real per-day cover any more (a card that would have shown one only ever carries the plain `.feed-day-card` class, never `--glass`), so per the four-combination table above ("covers off, glass on: EVERY day card gets the glass tint"), every card needs the same treatment or they'd mismatch against each other exactly as before, just with the roles reversed. `--landing-cover-url`/`--landing-cover-pos` (set on `document.documentElement`) are kept live by `covers.js`'s `_rotateLandingCover()`/`_applyLandingBgPosition()`.

**Day-card crossfade sync (added 2026-08-09):** `background-image` can't be transitioned by the browser, so the glass card's own background used to pop to the new cover the instant `--landing-cover-url` updated - well ahead of the real `#landing-bg-a`/`-b` page background, which crossfades over 1.5s. A `::before` layer (behind `.feed-day-content` and the `::after` dim overlay via normal `z-index: auto` source order, no explicit `z-index` needed) now holds the *previous* cover at full opacity via `--landing-cover-url-prev`/`-pos-prev`, fading it out over the same 1.5s `--landing-cover-fade` transition. `_rotateLandingCover()` captures the outgoing url/pos and pins the fade at `1` *before* swapping the base image, flushes layout (`offsetHeight` read), then flips the fade to `0` next frame - skipping that ordering risks the browser coalescing the 1→0 change with the initial 1 and never firing the transition at all. `body.no-feed-glass-cards` (the "Transparent background for day cards" toggle) also force-hides this `::before` layer alongside the existing background-image/`::after` overrides, so a stray previous-cover image can't flash through on cards where glass mode is fully disabled.

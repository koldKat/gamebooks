...
**Auto-nav:** a section with exactly one choice not yet in the run's path is auto-walked through without requiring a click, so a long chain of single-choice sections advances in one visual beat - **except** when that section also has a portal (`_owIsOpenWorld && secData.portals?.length`): a lone choice of `-1`/`0` (death/win) auto-navigates by immediately calling `endPlaythrough()`, which would resolve the run before the player ever got a chance to use the portal instead - so a portal on the section suppresses auto-nav and shows the manual choice button, letting the player pick either.

...
Layer 3 (feature modules - import from layers 0–2 as needed):
  notes.js, battlesim829.js, battlesim8.js, battlesim286.js, battlesim198.js, battlesim199.js, battlesim200.js, auth.js, add-book.js, edit-book.js,
...
`server.js`'s `computeCodeStats()` (feeds "Lines of code"/"Code size" in Stats for Nerds) scans the whole `public/css/`/`public/js`/`admin` directories dynamically (`fs.readdirSync(...).filter(f => f.endsWith('.css'|'.js'))`), not a hardcoded file list - it's an IIFE that runs once at module load, so a new file needs a server restart to be picked up.
...
**Gotcha:** typing a `\uXXXX` escape directly into a file-edit tool's string parameter gets
silently decoded to the actual Unicode character before the tool ever sees it as text -
reading the live file's exact bytes is the reliable way to base an edit on existing escape
sequences rather than retyping them by hand.
...
**`run_depth` is the deliberate exception - stays index-based (`${bookId}:${i}` / `series:${owSeriesId}:${i}`), do not "fix" it to match the others.** `run_depth` (~25 XP) fires the instant any run has a non-empty path, i.e. essentially on creation, which costs nothing - a guaranteed-unique ref would make it directly farmable (create a run, get the XP, delete it, repeat). Plain index-based dedup is the intended behavior: re-creating a run at the *same* slot after deleting it correctly does not re-earn `run_depth`, while a genuinely new slot does. One-time correction scripts (`scripts/revoke_duplicate_run_depth.js`, `scripts/restore_run_depth_correction.js`, `scripts/backfill_run_reward_refs.js`) exist in the repo for historical record; do not re-run them.
...
**Deliberately staggered, not fired in the same tick as `loadFeed()`:** both the 60s poll and the SSE debounce call `loadFeed()` and refresh the App/Avg-Level bars off the same trigger, but `refreshAppXp()` is wrapped in `_refreshAppXpStaggered()` (`APP_XP_STAGGER_MS = 1200`) rather than invoked directly alongside `loadFeed()` - since the viewer's own personal XP bar tween (via `loadFeed()`'s own `scheduleRewardProfileRefresh(150)`) and the App/Avg-Level bars' tween would otherwise start from the exact same instant every time, and with very different durations (App/Avg bars scale off their own low level; the personal bar is `viewerLevel * 100ms`) that reads as one bar doing a fast burst then a slow crawl rather than two independent bars.
...
Tiles are full-width, natural-aspect-ratio, top-anchored within their slot - not shrunk to a fixed thumbnail size, which would change the visual character of the feature (a stack of real covers vs. a row of small thumbnails).
...
`0.25` is deliberately much lighter than `.feed-day-cover-stack::after`'s `0.9` - the ambient landing background showing through a glass card is already dimmed by its own baked-in gradient, so a card-level tint anywhere near `0.9` compounds to near-total darkness. Testing this needs care: the opacity transition on `#landing-bg-a`/`b` takes 1.5s, and demo mode never activates the ambient rotation at all (both layers stay at `opacity: 0`), so a quick check can misread a value as fine (or broken) before anything real has had a chance to show through.
...
**Closing the Ctrl+Y panel** (`#cover-tooltip-settings-overlay`): three ways in - pressing Ctrl+Y again, clicking outside the panel (own mousedown/click tracker in `initCoversPanel()`, separate from `main.js`'s `_mousedownOnOverlay`), or the `✕` button in the panel's heading row (`.modal-title-row`/`.modal-close-btn`, the same reusable classes every other modal's close button uses), wired to `_toggleCoverTooltipSettings(false)`.

**Recompute triggers - `ResizeObserver` is the authoritative one:** every `.feed-day-card[data-day-index]` is `observe()`d by a single module-level `ResizeObserver` that calls `_scheduleDayCoverRecompute()` whenever that specific card's rendered size actually changes, for any reason - a CSS-animated panel-collapse transition, a window resize, a font finishing its load and reflowing text, unrelated sibling content shifting the card. Supplementary immediate nudges also exist: `window.resize`/`fullscreenchange` (debounced via `requestAnimationFrame` plus a `setTimeout(..., 400)` follow-up) and an explicit `refreshDayCoverFlows()` hook wired into `prefs.js`'s `_setLandingPanelCollapsed()` and `applyPrefs()`'s initial-load panel-state toggle.

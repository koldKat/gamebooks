...
**Day-card crossfade sync:** `background-image` can't be transitioned by the browser, so `.feed-day-card--glass`'s own background swaps to a new cover instantly while the real `#landing-bg-a`/`-b` page background crossfades over 1.5s. A `::before` layer (behind `.feed-day-content` and the `::after` dim overlay via normal `z-index: auto` source order) holds the *previous* cover at full opacity via `--landing-cover-url-prev`/`-pos-prev`, fading it out over the same 1.5s `--landing-cover-fade` transition. `_rotateLandingCover()` pins the fade at `1` before swapping the base image and flushes layout before flipping it to `0` next frame - required ordering, or the browser can coalesce the 1→0 change and skip the transition. `body.no-feed-glass-cards` also force-hides this `::before` layer alongside the existing overrides.

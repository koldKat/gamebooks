...
**UI:** a floating panel (`liveread-panel`, `public/css/liveread.css`), docked left, internally scrollable, toggled by an explicit button in `#play-btn-row`. Deliberately not built on the `.inv-overlay` blocking-modal pattern used by every other panel in the app - it has no full-screen backdrop, so the graph stays visible and interactive underneath while reading. Height is pinned between `top: 12px` and `bottom: calc(1rem + var(--play-btn-row-h, 2.1rem) + 0.4rem)` (the same variable `#stats-hud` uses to clear the button row/XP bar) rather than vertically centered with a flat `max-height` - centering plus a fixed height cap didn't know about the bottom bar's actual height, so on shorter viewports the panel could overlap `#play-btn-row`/`#play-bottom-stack`.
...
**Validation checklist for a new (or re-imported) `book_sections` file** - a real broken link (book 868's chapter 28) shipped once from skipping the last of these, so all five run on every import, not just the first:

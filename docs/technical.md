...
**Forum edit save losing markup formatting (2026-07-29):** the markup engine added above only runs server-side inside `renderBody()`. Editing a thread/post re-renders the body in-place client-side without a page reload (for a snappier save), but that path called `_escBr()` - a plain HTML-escape + `\n`→`<br>` helper with no markup parsing - so a saved edit displayed the raw `**`/`{color:...}` tags literally instead of the formatted result, even though the underlying stored body and any *fresh* page load rendered it correctly. Added `_renderBody()`, a client-side mirror of `renderBody()`'s regex chain (kept in sync by hand, same as `_escBr` already was), and pointed both the OP-edit and post-edit save handlers at it instead. Note the client script is itself embedded in a server-side template literal, so every backslash in the mirrored regex needs doubling to survive that round-trip intact - verified by extracting and syntax-checking the actual rendered `<script>` output, not just eyeballing the source. While fixing this, also found the thread-title edit only ever updated the big `#thread-title-display` heading - the breadcrumb (`Forum › Category › Title`) and the browser tab's `<title>` were both baked in once at page render and never touched by the edit handler, so they stayed stale until a reload. Gave the breadcrumb's title segment an `id="breadcrumb-title"` and updated both it and `document.title` alongside the heading.

**Announcements can link to a book, opening its detail dialog in-app (2026-07-29):** no markup-engine change needed - `formatAnnBody()` already renders `[Label](https://...)` links, and `/book/:id` (`publicBookPageRe` in `server.js`) is already a real, crawlable page (same one used by the no-JS `GET /feed` SEO page's book tags). So an announcement author just writes `[The Warlock of Firetop Mountain](https://koldkat.net/book/198)` like any other link. Added a click-interceptor in `feed.js` (alongside the existing `.feed-series-tag` one, same pattern) that, for any rendered link inside `.feed-ann-body`/`.feed-pinned-body` whose resolved URL is same-origin and matches `/book/(\d+)`, calls `preventDefault()` and opens `openCoverActivity(bookId, ...)` instead of navigating away - so the link still degrades correctly (real page, works logged-out, no-JS, opened in a new tab) if the interceptor doesn't fire for any reason.

**Themed link colors, and a redundancy caught while doing it:** links rendered by `formatAnnBody()`/`fmtAnnBody()` had no author styling, so they fell back to the browser's default dark blue/purple-visited - themed to the app's existing `#60a5fa` accent (already used for `nav-btn`, admin badges, etc.) in `demo.css` (`.feed-ann-body a`/`.feed-pinned-body a`) and `admin/index.html` (`.ann-card-body a`), neither of which had any prior `a` styling to conflict with. Went to add the same fix to `server/forum.js`'s `.post-body` and the no-JS SEO page's `.ann-body`/`.pinned-body` in `server/routes/public.js`, but both of those pages already had a global `a { color: #60a5fa; }` rule - so the scoped additions there were pure redundant duplication, not an actual fix. Removed them and added the one genuinely missing piece (`a:visited`) to each page's existing global rule instead.

**Forum book links forced a new tab and baked in one hardcoded domain (2026-07-29):** `renderBody()`'s `[label](url)` support only ever accepted `https?://` and always rendered `target="_blank"`, so a book link inside a forum post had to be a full absolute URL and always popped a new tab - wrong on both counts: this app is served from several domains (koldkat.net, pathmap.net, bookplay.net) so a baked-in domain is only correct on one of them, and an "obviously in-app" link shouldn't leave the page at all. Fixed by extending the link regex to also accept a relative `/book/\d+` path; those render *without* `target="_blank"` and get a `data-book-id` attribute instead. A new click-interceptor in the shared header script (`shell()`, so it's on every forum page) catches clicks on `a[data-book-id]` and reuses the existing `data-open-app` → `postMessage` bridge to the parent window (`{ type: 'gamebooks-open-book', bookId }`), which `boot.js`'s message listener now handles by closing the forum modal and calling `openCoverActivity(bookId, '')` - the real in-app book dialog, no navigation at all. If the forum is ever opened outside the app's iframe (no `window.parent`), the relative href still works as a plain same-tab link to the real `/book/:id` page. Genuine external `https://` links are untouched and still open in a new tab, which is correct there. The one live forum post using this syntax was also fixed to use `/book/198` instead of the hardcoded `https://koldkat.net/book/198` it was seeded with. The exact same underlying flaw was also present in `formatAnnBody()` (`feed.js`, duplicated in `admin/js/announcements.js`) and the no-JS SEO page's `fmtAnnBody()` (`server/routes/public.js`) - all three required a hardcoded absolute URL for a book link, which would silently resolve to the wrong domain's site when viewed from any domain other than the one baked into the text (the earlier same-origin click-interceptor doc entry above undersold this: on a domain mismatch it wasn't a harmless degrade, it was a full navigation to a different domain entirely). Extended all three the same way - relative `/book/\d+` accepted alongside `https?://`, rendered without `target="_blank"`. Also had to sync the same regex change into `_renderBody()`, the client-side mirror added earlier for post-edit re-rendering (see the forum-edit-losing-markup entry above) - missed on the first pass, which would have meant a `/book/123` link silently broke back into literal `[label](url)` text the moment its post was edited. Two live rows were already using the old hardcoded-domain syntax and got fixed directly: the one forum post, and announcement #59 (a changelog post the user had already published from an earlier draft in this session, using the hardcoded-URL syntax that draft was given in) - `announcements` table has no other affected rows, `forum_posts` (replies) has none either.

**Opening a book from a forum link closed the forum entirely (2026-07-29):** the `gamebooks-open-book` handler above was copy-pasted from `gamebooks-open-app`'s pattern, which calls `closeForumModal()` first - fine for "go to the app" (a real navigation away from the forum), wrong for "show this one book's detail dialog," since `openForumModal()` always resets the iframe to `/forum` home on reopen (see the "reopening to a stale page" fix above - that fix made reopening *correct* by resetting instead of leaving it stale, but this flow needed the forum to *not* be reset at all). User: "if i click a link on the forum, and it opens a popup, and then i close it, the forum is gone and i have to reopen it? does that sound right to you?" Root cause once traced: `#public-modal-overlay` (`.pub-overlay`) sits at `z-index: 300`, well below `#forum-modal-overlay`'s `3000`, so simply not closing the forum would've rendered the book dialog invisibly behind it - `closeForumModal()` was a workaround for that stacking problem, not a deliberate UX choice. Fixed properly: the forum stays open, and `#public-modal-overlay` gets a one-off inline `z-index: 3001` bump (above the forum) set right before `openCoverActivity()` is called, reset back to the CSS default in `closePublicModal()` so it doesn't linger for the many other flows that open this same modal outside the forum context. Verified every close path (X button, click-outside, Escape) routes through `closePublicModal()`, so the reset is never skipped. Leaving the forum open uncovered two more of the same stacking problem one level deeper: from that same dialog, "Open Owned Book" calls `navigateToBook()` (a real navigation into the play/graph screen, no z-index of its own - would render *behind* a still-open forum) and, for admins/authors, "Edit" opens `#edit-book-modal-overlay` (`z-index: 2000`, also below the forum's `3000`). Audited every hook reachable from the single-book cover-activity dialog (`_hooks.navigateToBook`, `_hooks.openEditBookModal`, `_hooks.showBooks` - the last one only reachable from the *series* activity dialog, not this flow) and confirmed no other overlay gets opened outside that hook system. Added the same one-line `document.getElementById('forum-modal-overlay')?.classList.remove('active')` to both `navigateToBook()` (`boot.js`) and `openEditBookModal()` (`edit-book.js`) - both are "leaving the quick-look, doing something real" actions, same reasoning as the original `gamebooks-open-app` case, just reachable one layer deeper than the dialog itself. Turned out the "no other overlay reachable" audit missed one - the single-book dialog can navigate *internally* (same overlay, no `closePublicModal()` in between, so the earlier z-index bump survives correctly) into the *series* view via its "Series" tag, and that series view's own "Add series to library" button calls `_hooks.showBooks()` - the exact path already dismissed as "only reachable from the series dialog, not this flow," except the single-book dialog can reach the series dialog too. Rather than keep patching individual call sites as more get discovered, fixed it at the actual choke point this time: `showBooks()` itself (`boot.js`) now unconditionally closes the forum as its first action, since a freshly-shown books screen should never have a forum modal left sitting over it regardless of which of its 6 call sites got there. The `gamebooks-open-app` handler's own explicit `closeForumModal()` before its conditional `showBooks()` call is now redundant in the case where `showBooks()` fires, but still load-bearing for the anonymous-user branch where it doesn't - left both in place. One more caught right after: `openEditBookModal()` deliberately leaves the underlying public book dialog open (Cancel/Save should land back on it), so closing the forum there wasn't the whole fix - if that dialog was opened from the forum, its z-index is still sitting at the temporary `3001` bump, which is now *higher* than the edit modal's own `2000` and would render it invisibly behind the still-elevated (but no-longer-relevant) dialog. `navigateToBook()`'s one caller already handles this correctly since it calls `closePublicModal()` right after (which resets the z-index and fully closes the dialog, appropriate there since there's nothing to return to). `openEditBookModal()` needed its own explicit reset instead, since a full `closePublicModal()` would also wipe the dialog's title/body/network state that Cancel/Save needs to return to intact.

**Demo mode: opening Create Book/Anthology/Series silently logged the demo out (2026-07-29):** all three "Create" dialogs fire an authenticated `apiFetch` call the moment they open, to populate their series dropdown/autocomplete - `_populateSeriesSelect()` (`edit-book.js`, called unguarded from both `openAddBook()` and `openAddComp()` in `add-book.js`) hits `/api/series`, and `openAddSeries()` hits `/api/series/autocomplete` directly. Demo mode has no real account or token, so these always 401 - and `apiFetch()`'s 401 handler (`state.js`) unconditionally calls `clearToken()`/`clearUsername()` and dispatches `auth-expired`, which `boot.js` wires straight to `showLogin()`. All of this happens silently behind the still-open modal, so the demo user only notices once they close it and the login screen (already revealed) is sitting there - the demo banner up top is untouched since it's separate state, giving the exact symptom reported: "the demo disappears, the bar remains at the top that i'm in demo mode, but on the right, the login appears." `edit-book.js`'s own Edit Book flow already guarded this exact call (`if (!isDemoMode) _populateSeriesSelect(...)` at its one call site) - the bug was specifically in the three *Create* dialogs never inheriting that guard. Fixed at the choke point for two of the three: `_populateSeriesSelect()` now early-returns under `isDemoMode` itself, covering both `openAddBook()` and `openAddComp()`'s unguarded calls (and making the pre-existing outer guard in the Edit flow redundant-but-harmless). `openAddSeries()` uses a different endpoint entirely, so it needed its own separate `isDemoMode` guard (new import into `add-book.js`, which didn't have it before). Series data isn't meaningful for the demo book anyway (it isn't part of any real series), so skipping the fetch entirely rather than serving cached/empty data is the correct behavior, not just a workaround. The three Save/Create buttons had the identical gap one step further in - clicking through despite the disabled autocomplete would still hit `POST /api/books`/`/api/series` directly, the same 401→logout. Guarded all three (`cb-save`/`cc-save`/`csr-save` in `add-book.js`) with the same `isDemoMode` check, surfaced via the existing inline `errEl.textContent` pattern each form already uses for validation errors (new `addbook.demo_not_supported` i18n key) rather than a separate alert dialog - consistent with how these forms already report problems. One more found while checking whether the demo's own book could reach an equivalent path: `books.js`'s `isCreator` check (`isDemoMode || b.created_by === null || b.created_by === _currentUserId`) treats demo mode as the creator of its fake book, so the pencil "Edit" button *is* shown and clickable on it - `openEditBookModal()` fires an unconditional `apiFetch('/api/books/${bookId}/rating')` the instant it opens (star-rating widget, same "silent 401 behind the modal" shape as the Create dialogs), plus the star-click handler and the Save button both hit their own authenticated endpoints if clicked. Guarded all three the same way - skip the auto-fetch and the star-click under `isDemoMode`, and the Save handler shows the same `addbook.demo_not_supported` message. `openEditCompModal`/`openEditSeriesModal` don't need the same treatment - the demo's one fake book isn't a container or part of a series, so neither is reachable from it.

One more caught on review: `admin/js/announcements.js`'s copy of this regex change also dropped `target="_blank"` for the relative case, matching the player-facing version - but the admin composer preview has no `openCoverActivity`/click-interceptor to open a book into (it's a separate app with nowhere in-app to send the click), so a relative link there would navigate the whole admin tab away to the public page instead of opening in a new tab like it used to. Reverted that one spot to always use `target="_blank"` regardless of relative/absolute - the player-facing feed/forum are the only two contexts with somewhere better to send the click.
...
**Browser autofill competing with the battle-sim enemy-name dropdown (2026-07-29):** all four sims' enemy-name field (`battlesim198.js`/`286.js`/`829.js`/`8.js`) is a plain `<input autocomplete="off">` paired with its own absolutely-positioned `<ul class="autocomplete-dropdown">` suggestion list - but `autocomplete="off"` is well known to be ignored by Chromium's own autofill/form-history heuristics for fields it classifies as fillable, so the browser's native suggestion popup could still appear stacked on top of (or fighting for the same space as) the app's own dropdown. Fixed by giving the widget proper ARIA combobox semantics instead of relying on the `autocomplete` attribute alone: `role="combobox"` / `aria-autocomplete="list"` / `aria-haspopup="listbox"` / `aria-controls` on the input, `role="listbox"` on the `<ul>`, `role="option"` on each `<li>`, and `aria-expanded` toggled true/false alongside the existing `.open` class (centralized into a new shared `closeDropdown()` helper in each file, replacing every direct `dropdown.classList.remove('open')` call so the two states can't drift apart). Browsers use these roles as a signal that the page already provides its own listbox UI and suppress their native one accordingly - a real fix rather than an `autocomplete` value hack, and a genuine accessibility improvement on top. Caught on review that the first pass added the roles but never wired `aria-activedescendant` - the actual mechanism a screen reader uses to announce which option is highlighted as arrow keys move `activeIdx` (focus stays on the input the whole time in this "list" combobox pattern, so `aria-activedescendant` is how the highlighted option gets communicated at all). Half-adding ARIA roles without it would arguably be worse than not adding them - announcing "combobox" but never saying what's selected. Completed it: each `<li>` gets a stable id (`{dropdown-id}-opt-{i}`), and the keydown handler sets/clears `aria-activedescendant` on the input alongside the existing `.ac-active` class toggle, cleared on close/re-render via the same `closeDropdown()`/`render()` functions. Also added the companion `aria-selected="true"/"false"` on each `<li>` itself (per the WAI-ARIA APG combobox-with-list-autocomplete pattern, `aria-activedescendant` on the input and `aria-selected` on the pointed-to option are meant to travel together) - not set on initial render since an omitted `aria-selected` already defaults to false for spec purposes, only toggled from the same keydown handler alongside `.ac-active`.

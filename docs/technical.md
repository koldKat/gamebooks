  ...
  public/
    ...
    js/
      ...
      battlesim/         All battle simulator modules, one file per book, grouped in their own
        ...
        battlesim224.js    Battle simulator for book 224, Phantoms of Fear (Fighting Fantasy) - plain FF SKILL/STAMINA/LUCK, confirmed from this book's own rules text. Adds a fourth stat, POWER, for a dream-control spell system that can resolve a fight instead of rolling it out - an alternate resolution path (skip the dice fight via a printed spell instruction), not a per-round combat modifier, and not modeled, same as this app's treatment of narrative branch choices generally. No Provisions/Potions mechanic found in the rules text. This book's own PDF scan had unusually heavy OCR corruption (the embedded text layer was a broken/garbled font-encoding baked into the source file, requiring a fresh Tesseract OCR pass rather than a direct pdftotext extraction) - 40 of 400 book_sections rows are honest "not located" placeholders, and several combat encounters had SKILL/STAMINA digits too garbled to confidently decode (single-character OCR noise standing in for a digit, e.g. "§"/"&"). Those encounters (a paired First/Second Orc fight at §13, Giant Sloodkitawk at §29, a Bear entry inside a random-encounter table at §144, a paired Troll/Ogre fight at §7, a second weakened Troll at §385, plus Giant Scorpions/a lone Dark Elf/Ferry Man/Roots mentioned in the text but not locatable to a specific confidently-read stat line) were excluded from book_enemies entirely rather than guessed. book_enemies (20 rows) uses the same .attack/.hp convention as every other FF sim; two different-named creatures reusing the same name at different encounters (Pygmy Orc, Weevil Man, Wild Man) are kept as separate rows disambiguated by section number, since Pygmy Orc's two encounters have genuinely different SKILL/STAMINA while Weevil Man's and Wild Man's are identical repeats.

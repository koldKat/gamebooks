  ...
  public/
    ...
    js/
      ...
      battlesim/         All battle simulator modules, one file per book, grouped in their own
        ...
        battlesim286.js    Battle simulator for book 286 (flat weapon min-hit model, tech gadgets, sleep/dream table; re-verified against the full book text 2026-08-19 once live-reading content became available - fixed a swapped dream 9/11 label+text pair, added enemy.fixedDamage for the handful of enemies whose text states a flat per-hit amount instead of the usual roll-minus-minimum, and enemy.extraAttackers generalizing this app's existing "unwoundable second attacker" pattern to N simultaneous/sequential attackers for the book's four multi-enemy fights (secs 13/15/29/173) - none of this was modeled at all before, since no full/clean source existed to catch it)
...
| GET | `/api/books` | List user's books → `[{ id, name, total_sections, discoverable_sections, isbn, issn, asin, cover_path, pages, authors, description, created_at, created_by, is_public, visited, series_id, series_number, series_name, is_container, parent_book_id, book_order, bgHidden, bgPosY, extra_anthology_ids, extra_anthology_orders }]`. `visited` is the count of unique section numbers that appear in any playthrough's `path` array. Computed server-side by parsing `state_data`. `series_name` is joined from the `series` table. `bgHidden`/`bgPosY` come from `user_books.bg_hidden`/`user_books.bg_pos_y`. `extra_anthology_ids` is the book's *secondary* anthology memberships (see `book_anthology_memberships` below) - `parent_book_id` remains the one primary anthology. `extra_anthology_orders` is `{anthologyId: book_order}` - each secondary membership has its own order scoped to that anthology, distinct from the book's own `book_order` column (which is only its position within its *primary* parent). |
...
**`books.has_battle_sim`** (DB column, added 2026-08-08) is the source of truth for "does this book have a battle sim," used by `books.js`/`covers.js` for the library/covers-wall badge and filter (`getBooks()` returns it straight through; `getAllPublicBooks()` also returns a container-aware `hasBattleSim` that's true if the book itself or any of its anthology children has the flag, via `MAX(c.has_battle_sim)` in the same `GROUP BY` query). Previously this was a plain array (`BATTLE_SIM_BOOK_IDS` in `util.js`) that had to be kept in sync by hand with `boot.js`'s per-book panel-visibility gating - that array is gone now. `boot.js`'s gating itself (`setBattleSimVisible(bookId === 829)`, `setSim8Visible(bookId === 8)`, etc., one line per sim) is unchanged and still hardcoded - each sim is its own bespoke module boot.js has to dispatch to by ID, which the DB flag doesn't remove (it only tells you *whether* a book has a sim, not *which* module handles it). Adding a new sim still means adding a `setSimNVisible()` line in `boot.js` plus setting `has_battle_sim = 1` for that book (currently via a manual `UPDATE`, no admin UI toggle yet), but no longer means touching a second hardcoded ID list too.
...
- **XP**: adding a secondary membership reuses the primary anthology's own events - `add_book_to_anthology` (10, first time `addAnthologyMember` creates a new row for that book/anthology pair) and `add_anthology_order` (5, whenever `book_order` is set on that call) - paid to the *book's* creator (`result.bookCreatedBy`), not whoever's adding it (usually the anthology's creator, who may not own the book). Both events are deduped per `(user, event, ref)` with `ref = "${bookId}:${anthologyId}"` - **not** bare `bookId` - since the primary attachment (`handleCreateBook`/`handleUpdateBook`) already claims the bare-`bookId` ref for these same two events; reusing it here would silently pay zero XP for every secondary membership on any book that's ever had a primary anthology (i.e. almost always). Re-editing a membership's order via the edit-book chip doesn't re-award `add_anthology_order` after the first time for that same anthology pair.
...
**Reward dedup refs:** `death_run`/`battle_run`/`win_run`/`share_run`/`charsheet_run`/`charsheet_saved` key their `xp_events` dedup ref off the run's `startedAt` timestamp (`newPt?.startedAt ?? i`, array index only as a fallback for runs that predate this field) - `startedAt` is assigned once at creation and never reused, so it can't collide the way an array index can once a run at that slot is deleted and a new one takes its place. The ref's **prefix** additionally depends on whether the book was in an open-world series *at the moment that specific run completed*: `bookId:startedAt` normally, `series:seriesId:startedAt` for an open-world one (`processStateXp`) - so a single book's history can contain both formats (e.g. losses from before it joined a series, plus later ones after). Anything that needs to find "the run behind this ref" (`getFeed()`'s `first_win`/`first_loss`/`first_battle_death` resolution, in particular, via `_getFirstRunRef`/`_runKeyFromRef`) must check both prefixes, not just the book-scoped one - matching only `bookId:%` silently fails to link an open-world-native first win/loss to any run at all.

**better-sqlite3 gotcha:** never concatenate a bound JS-number parameter into a `LIKE` pattern in SQL (`'series:' || ? || ':%'` with a number bound to that `?`) - better-sqlite3 binds a plain JS number as SQLite `REAL` unless it's told otherwise, and concatenating a `REAL` stringifies it with a trailing `.0` (`'series:123.0:%'` instead of `'series:123:%'`), so the pattern silently never matches. `CAST(? AS TEXT)` does **not** fix this - casting an already-`REAL`-bound value still produces `"123.0"`. The only real fix is to `String()` the value in JS before binding it, so it binds as `TEXT` to begin with. This exact bug shipped in `_getFirstRunRef`'s `seriesId` param (`server/db/feed.js`) and caused a real user-visible duplicate: `first_win`/`first_loss` silently failed to resolve their `runIndex` for any open-world-native completion, which meant the `series_run_completed` de-dup (keyed on matching `bookId`+`runIndex`) never fired either, showing "won series run N" and "won ... for the first time" as two separate feed entries for the same event. Building the whole pattern string in JS first (`${bookId}:%`, as `_permanentVisitedCount`/the reconciliation safety net/`resetBookProgress` all already do) sidesteps this entirely and is the preferred style - reach for it before ever writing `'prefix:' || ? || ':%'` in SQL.
...
**Security: the forgot-password reset link must never be built from `req.headers.host`** - it's attacker-controlled, and this link is emailed to the account owner, so a spoofed `Host` header would poison the reset link toward an attacker's domain (Host Header Injection / password-reset poisoning, a real account-takeover path). The link is built from `db.getAdminSetting('app_url')`, hardcoded to `'https://pathmap.net'` if unset (changed 2026-08-16, previously `koldkat.net` - koldkat.net is planned to eventually become a separate personal portfolio site, unrelated to this app, so pathmap.net is the forward-looking default) - never a header-derived fallback. Also: `handleForgotPassword`'s `429` rate-limit response must be checked via `res.ok` before assuming success, same as the other three auth handlers in this file.
...
- `GET /api/feed` is a pure read with no side effects (fixed 2026-08-16 - it used to also award `idle_heartbeat` XP here, which broke the "GET is side-effect-free" expectation and, worse, meant every SSE-triggered feed reload fired by *other users'* activity granted this user an extra heartbeat/bonus-coin-roll opportunity too, bursting in sync with site activity rather than this user's own idle time). Heartbeat XP now only comes from `livetab.js`'s dedicated 60-second leader-tab timer hitting `POST /api/heartbeat` directly - see `idle_heartbeat` in the XP events table above. `loadFeed()` still schedules a short profile refresh after every call (`scheduleRewardProfileRefresh(150)`) so the coin/XP floater picks up whatever the heartbeat timer most recently did, but `loadFeed()` itself no longer *causes* anything.
...
All three skip the entry entirely (not just degrade it) when `_resolveRunIndex` can't find the run in current state - the XP/achievement itself is permanent (deleting a run never revokes XP already earned), but a feed entry with nothing left to link to or show a path for is worse than no entry, not a lesser version of one. This matters in practice: a run that's been deleted since it earned the achievement resolves to `runIndex == null` and is now filtered out, rather than showing a dangling, unclickable "won for the first time" for a run that no longer exists anywhere in the player's own data.
...
**Completed-run counts read from the permanent `xp_events` ledger, not live `state_data` (changed 2026-08-18).** `getProfileStats()` (`server/db/feed.js`), `adminGetUsers()`, and `adminGetUserBooks()`'s summary `totals` (all `server/db/admin.js`) each `GROUP BY event` (and, for `adminGetUsers()`, `user_id` too) over `xp_events WHERE event IN ('win_run','death_run','battle_run')` - the same source the runs-milestone GC coin (`server/db/xp.js`, `_anyRunJustCompleted` block) already used. They used to re-derive the count live by re-scanning every book's `user_books.state_data` for playthroughs with a terminal `result`, which drifted downward over time: a book removed from the library, or a playthrough array later pruned/reset (`resetBookProgress`, a series-run deletion), silently dropped an already-earned, already-paid-out run from the total forever, disagreeing with the milestone that had already fired for it. `adminGetUserBooks()`'s **per-book** `playthroughs`/`wins`/`deaths`/`battles` numbers are unchanged (still state_data-derived, which is correct for "what does this one book currently show") - only the page's overall summary header switched sources. `adminGetUsers()` still separately counts in-progress runs (`!pt.result`) as `active` from live state_data, shown in the users table's own "Active" column, since there's no permanent event logged for "started but not finished." All of these aggregates (plus `adminGetBookStats()`'s `inProgress`/`totalPts` and `adminGetStats()`'s site-wide `activePlaythroughs`/`playthroughs`, which still are fully state_data-derived) exclude untouched open-world series-run placeholders (`pt.startedAt == null`) - every book in a series carries one padding slot per series run so numbers line up across books, and without this filter each of these counts was inflated by however many sibling books' padding slots the user's account happened to have, regardless of whether they'd ever actually played there.
...
- `server/db/xp.js`'s `SIM_HISTORY_KEYS` array - omitting this doesn't break the sim itself, just silently pays zero `battlesim_win`/`battlesim_loss` XP forever.
- `public/css/battlesim.css`'s two combined ID selectors (the base button style block and its `:hover` block) - **the trigger button has no generic `#play-btn-row button` fallback style, only this explicit per-ID list** (same pattern for `#charsheet-btn`/`#inventory-btn`/etc.). Omitting the new sim's `#simNNN-btn` from both doesn't break anything functionally - the button still renders and works - it just displays as an unstyled default HTML button instead of matching every other trigger button's dark pill styling.
- `book_enemies` rows seeded for the new `book_id` (via direct SQL, no admin UI) and `books.has_battle_sim = 1` set for that book.
...
`adminBadge()` (`user.js`) takes the already-resolved `isAdmin` boolean directly, matching `authorBadge`/`contributorBadge`'s own calling convention - it used to take a username and re-derive admin status itself by comparing against `_adminUsername`, but all 3 callers (`boot.js`, rendering `#books-username` - the currently logged-in user's own header) already have `_isAdmin` (`resolveIsAdmin()`'s result) sitting in scope, and that's strictly more reliable: `_isAdmin` is driven by `profile.isAdmin` the moment `/api/profile` resolves, while the username-compare path depended on `/api/config`'s `adminUsername` having *also* already arrived - a separate, unawaited fetch, so a real (if narrow) race that could leave the actual admin's own badge silently missing for a moment on login/profile-save even though `_isAdmin` was already correctly `true`.
...
**Hover image preview (`#feed-img-preview`, desktop only) click guard, added 2026-08-08:** hovering a `.feed-user`/`[data-cover]` element shows an avatar/cover thumbnail (plus level/title text for avatars) via `mouseenter`, hidden again on `mouseleave` via a shared `_hideFeedPreview()`. Clicking the hovered element to open a dialog (e.g. a feed avatar → public profile, or a cover → activity view) doesn't fire `mouseleave` - the pointer never actually leaves the element, it just triggers a click - so without an explicit guard the preview stayed floating on top of whatever dialog just opened, until the mouse happened to move again. Fixed the same way `tooltip.js`'s own `data-tooltip` system already guards against this: a single module-level `document.addEventListener('click', _hideFeedPreview)`, registered once at import time (not per `loadFeed()` render, which would otherwise stack a new listener on every SSE-triggered feed reload).
...
**Cover blob cache is capped at `_COVER_BLOB_CACHE_MAX` (60) full-size images, FIFO-evicted with `URL.revokeObjectURL()` on the loser (added 2026-08-18).** `_loadCoverWithProgress`/`_preloadCoverBlob` decode every fetched cover into a `Blob` + `URL.createObjectURL`, keyed by URL in a module-level `Map` for the rest of the tab session - previously uncapped, with no `revokeObjectURL` call anywhere, so every distinct cover ever scrolled past in a session stayed pinned in memory as a live decoded image for as long as the tab stayed open. On an account with a large personal library, this was the actual source of a reported ~300MB browser-tab leak, scaling with how much of the library had been scrolled/browsed, not with any single book. `_cacheCoverBlobUrl()` is the shared insert-and-evict entry point both callers now go through.

**`books.js`'s My Books list only loads covers for what's actually visible (changed 2026-08-18).** `_bookItemHtml()` renders every anthology/series/stash child into the DOM unconditionally - a collapsed group is hidden via CSS (`display:none`), not omitted - so `_queueBookCovers()`'s `container.querySelectorAll('[data-pending-cover]')` scan skips anything currently `offsetParent === null` (hidden by itself or an ancestor). Previously it queued and sequentially network-loaded every child's cover regardless of collapse state, which for an account where most books live inside anthologies/series meant eager-loading hundreds of covers nobody had asked to see. The three expand-toggle handlers (anthology/series/stash) and `_applyBooksSearchFilter()` (a search match force-reveals its container) each call `_queueBookCovers(group_or_list, { reset: false })` right after making their target visible, so newly-revealed covers still load - `reset: false` appends to whatever's already queued/running instead of bumping `_bookCoverGen` and dropping still-pending work from the initial pass.
...
**Every colour `nodeColor()` returns includes matching `highlight: {background, border}` and `hover: {background, border}`** (via the `_withHighlight()` wrapper at every return site), copying the node's own colours rather than leaving either unset. vis-network falls back to its own hardcoded default hover/select palette (`#D2E5FF` background, `#2B7CE9` border) for any node color object that doesn't specify these - none of the `COLORS.*` constants did, so simply hovering (`interaction.hover: true` is set) or selecting a node (any node, regardless of its real semantic colour) briefly repainted it with that unrelated generic blue instead of its actual colour, which read as a genuine bug rather than an interaction affordance (a currently-active orange node would flash pale blue on hover, looking like a rendering error). `highlight` applies on selection, `hover` on mere mouse-over - vis-network's own option parsing keeps them as two independent sub-palettes, so both need to be set or only one of the two interactions is actually fixed. The portal-specific colour override (further down, `isPortal ? { ...nodeColor(sec), highlight: { ...(nodeColor(sec).highlight || {}), border: ... } } : ...`) already anticipated a `highlight` key existing to spread from; it just never got one from the base function until this fix (it doesn't spread `hover`, so a portal node's gold cue only shows on its base fill and while selected, not while merely hovered - a pre-existing, unrelated minor gap, not something this fix introduced). Not reachable in the admin watch prototype (`admin/js/watch.js`'s port of this function) - its network is created with `selectable: false`, so nodes can't be hover/select-highlighted there at all.
...
**Debounced, not immediate (fixed 2026-08-16):** `render()` calls `syncGraph()` on every invocation, and several distinct user actions can each trigger their own `render()` within a few ms of each other (e.g. losing a run, marking it public, and starting a new one all chain through separate `saveState()`/UI-update callbacks). Before the fix, each of those calls restarted the physics solver from scratch immediately, interrupting a not-yet-finished `stabilize(300)` pass before it could ever fire `stabilizationIterationsDone` - the graph could then visibly re-jostle indefinitely, worse the more fixed obstacles (e.g. manually-added nodes, which get a saved position immediately and `physics: false`, but still count toward `avoidOverlap` collision-avoidance) the solver has to route around, since more obstacles means a slower convergence that's more likely to still be running when the next render arrives. `_restabilizeTimer` (a plain `setTimeout`, cleared and rescheduled on every qualifying `syncGraph` call, and cleared in `destroyNetwork()` to avoid a stray callback touching a torn-down network) means only the last call in a tight burst actually kicks off a pass.

**Debouncing alone wasn't enough (fixed same day, follow-up).** The debounce only guards against *scheduling* a new pass too soon after the last *request* - it says nothing about whether the *previous* pass has actually finished *running* yet. A graph with several fixed obstacles can take longer than the debounce window to converge, so a later debounced call could still fire while an earlier `stabilize(300)` was genuinely still in flight, kicking off a second concurrent physics pass that independently repositions the same nodes on top of the first - visible as continuous chaotic jitter, confirmed via a user report that it stopped entirely with DevTools open the whole time (added JS execution overhead widened timing enough that the first pass always finished before a second could be scheduled - same "a print statement masks a race condition" pattern as in threaded code). Fixed with a `_stabilizing` flag, true only while a pass is genuinely active: if the debounced callback fires while `_stabilizing` is already true, it doesn't start a second pass - it just reschedules itself to check again, letting the in-flight pass's own completion handler be the one to actually clear the flag. Reset in three places, all necessary: the completion handler (normal case), `destroyNetwork()` (book switch mid-pass), and the `else if (hasSavedPositions)` branch (force-disabling physics while a pass is still active would otherwise leave `_stabilizing` stuck `true` forever, silently blocking every future restabilize attempt for that book).
...
- **The destination is itself a terminal (`isTerminal(secData.choices[0])`, i.e. `-1`/`0`, death/win):** never auto-navigated into, regardless of portals - a book that happens to be mapped as a straight, all-single-choice line from the start section into a death or win (nothing branching, nothing to click) would otherwise auto-resolve the entire run in one silent beat with zero player interaction. Requiring a click on the final death/win step gives the player a chance to actually see and register the outcome before deciding to start again, rather than a run resolving and vanishing faster than they can perceive it - which also matters for `getFeed()`'s `first_win`/`first_loss`/`first_battle_death` (sourced from the immutable XP ledger, not live state): a player rapid-firing "New Run" against an instant-resolving book can rack up several real completions before ever realizing what happened, and a client-side save race under that kind of rapid-fire burst can leave live state out of sync with the ledger even though every individual completion was awarded correctly.
...
**Graph viewport jitter (fixed 2026-08-17):** `navigate()` skips its `network.focus()` camera-pan call whenever `wouldAutoNav()` says the landed-on section will be immediately auto-navigated away from, because stacking multiple 1s vis.js focus animations back-to-back corrupts vis-network's internal camera-animation state - it can keep bouncing between several distinct viewport positions/scales for several seconds, sometimes not settling until the page is refreshed and the `Network` instance is recreated from scratch. That guard originally existed only in `navigate()`. Every other `network.focus()` call site that fires right after a `render()` on an active playthrough - `startPlaythrough()`, both `startPortalRun()` sites, `loadRun()`, `undo()` (all `play.js`), fast-travel's `doJump()` (`boot.js`), and three sites in `open-world.js` (series-run-start, `_focusNodeAfterLoad()`, cross-book fast-travel) - lacked it, so any of them could race against `navigate()`'s own auto-nav chain and reproduce the same corruption. Fixed by routing all of them through the shared `wouldAutoNav()` export. A few call sites are deliberately left unguarded because they're one-off explicit user actions outside any auto-nav chain, not part of the render-then-focus race: `center-current-btn`, find-node's Enter/click handler (both `boot.js`), and the pre-series-run trail viewer (`play.js`).

**Second jitter path - stale `sec` in `_focusNodeAfterLoad()` (fixed 2026-08-18):** `showMain()` (`boot.js`) reads `currentSection()` synchronously right after `render()` and passes it straight into `_focusNodeAfterLoad(sec)`. If that section starts a straight-path auto-nav chain, the chain resolves asynchronously (`renderPlaythroughPanel()`'s own chained `setTimeout(0)` hops, `play.js`), not within that same synchronous call - so `sec` is captured *before* the chain has moved anywhere. By the time `_focusNodeAfterLoad`'s own 50ms-later `doFocus()` check runs, the chain has usually already advanced `pt.path` past that stale section, which makes `wouldAutoNav(sec, pt)` (checked against the now-stale `sec`) incorrectly read as "not auto-navving" (`pt.path` now already contains what was `sec`'s one next choice) - firing its own `focus()` at the wrong, already-passed-through node at the same moment the chain's own correctly-guarded final-hop `focus()` fires at the real current section. Two stacked animations, the same underlying corruption as above, reached through a path the original guard didn't cover (reopening a book whose active run sits at an unresolved chain start - e.g. a new run started, then leaving before its own auto-nav had a chance to run). Fixed by having `doFocus()` re-derive `currentSection()` fresh at fire time (falling back to the passed `sec` only if that's null) instead of trusting the value it was scheduled with.
...
**Font size:** two header buttons (`#liveread-font-dec`/`-inc`, styled off `.inv-close-btn`, not a native `<input type="number">`) plus a live `#liveread-font-pct` readout ("− 100% +") step `.liveread-body`'s font size between 70%-130% in whole 5% increments. Stored and stepped as a whole percent (`liveread-font-pct` in `localStorage`, same tier as `trailCollapsed`), not as a raw rem value - the first version stepped in `0.08rem` against a `0.88rem` base, which produced ugly non-round percentages (`0.08/0.88 ≈ 9%` per step, and a `147.7%` max that only looked like a bug once someone actually did the division). The percent is the source of truth; rem is derived from it (`FONT_SIZE_BASE_REM * pct / 100`) only when writing the panel-scoped `--liveread-font-size` CSS var (`panel.style.setProperty`, not global, so it can't leak into another panel sharing `liveread.css`).

**Reading font:** self-hosted PT Serif (`public/vendor/fonts/pt-serif/`, ~450KB across 16 woff2 subset files, vendored the same way vis-network is rather than pulled from a CDN at runtime) replaced Times New Roman as `.liveread-body`'s font-family. Times New Roman has full Cyrillic coverage (unlike Georgia, dropped earlier for exactly that reason) but was never actually hinted for on-screen reading, only print - PT Serif was designed for screens specifically and carries native, not bolted-on, Cyrillic support, so it fixes both the Cyrillic and the general-readability complaint at once. `.woff2`/`.woff` needed adding to `server/runtime-state.js`'s `MIME` map - static serving had no registered MIME type for either before this, so requests would have fallen through to `application/octet-stream`.
...
**Trigger button lives in `#play-btns-bar`, not `#play-btn-row` (moved 2026-08-18):** every other panel's trigger (`#charsheet-btn`, `#inventory-btn`, each `#simNNN-btn`) gets dynamically appended to `#play-btn-row` (`charsheet.js`'s `getPlayBtnRow()`), but that row can hold 4-5 buttons at once (Equipment/Inventory/Character Sheet/Battle Simulator) and a `flex-wrap`/`flex-direction:row-reverse` interaction there could stack a button on top of a neighbor instead of cleanly wrapping it to a new row once `#liveread-btn` joined them (fixed generally with `flex-shrink:0` on every button in that row, but `#liveread-btn` was moved out entirely rather than relying on that alone). It lives as static markup in `index.html`, between `#guide-btn` and `#notebook-btn` inside `#play-bottom-stack`'s `#play-btns-bar` - a row that only ever has 2-3 buttons, so there's no crowding risk there. Styled alongside its new siblings in `charsheet.css`, not with its own rule in `liveread.css` the way the old `#play-btn-row`-based version needed.
...
**Positioning history:** several earlier attempts docked it left, alongside `#run-trail-float`/`#notes-display`/`#dice-roller-wrap`, which all already live in that same column (top/middle/bottom respectively) - a panel meant to stay open for a whole reading session ended up covering one of them no matter how it was sized (vertically centered with a flat cap missed the bottom bar's real height on short viewports; a top/bottom-anchored version fixed that but then fully covered `#run-trail-float`; a `--play-btn-row-h`-based bottom calc was wrong anyway since that variable tracks the *right*-docked button row, not the left-docked dice roller it ended up covering instead). Moved right, docked under `#legend` (`right: 12px; top: calc(var(--legend-h, 260px) + 24px)`) - but the first version of *that* also anchored `bottom` to `#play-btn-row`, which just reproduced the same full-column-span bug on the new edge. Final: `top` anchor only, `max-height: 50vh` - anchoring both ends of a floating panel to its neighbors reliably recreates the "takes up all the space" bug regardless of which edge it's on; a single anchor plus a capped height doesn't. `--legend-h` is tracked by a `ResizeObserver` in `liveread.js`'s `initLiveRead()` (mirrors `charsheet.js`'s existing `--play-btn-row-h` tracking) since the legend's real height varies with its collapse toggle and the portal legend row's visibility.
...
**Non-choice in-text links ("extra" content):** source HTML occasionally links to something that isn't a real, choosable section - an unnumbered closing "Epilogue" some books tack on after their actual win section (book 708), or a rules-reference link repeated inline (`#rules`, `#rule-NNN`, book 186/708). Importing one of those as a normal `#section-N` target would register it as a graph node/choice it isn't - the epilogue is just bonus prose hung off an existing win section, not a new decision point. `_onChoiceClick` only routes `href^="#section-"` links through `navigate()`/`commitChoices()`; any other in-text `#...` link is treated as pure text and handled by `_showExtra()`, which fetches that key from `book_sections` (same table, same per-book text/choices row, just never referenced by another section's `choices` array) and renders it inline with a Back link - `_shownSec` is left untouched throughout, so nothing is written to `state.graph`/`pt.path` and the graph is unaffected. Before this, an unmatched link fell through to the browser's own default anchor handling and could trip `boot.js`'s `popstate` route handler, dumping the reader back on the app's home screen.

**Terminal sections:** -1/0 have no `book_sections` row; the panel shows a plain end-state message instead of fetching. `renderLiveRead()` falls back to `viewingPt.result` when `currentPlaythrough()` is null - `endPlaythrough()` clears `activePtIndex` and fires `setOnViewingPtChange` synchronously *before* the panel's own post-`navigate()` refresh runs, so without this fallback the panel would close itself instead of showing the win/death message.

**Following navigation from outside the panel:** `renderLiveRead()` is wired into *two* hooks, not one - `boot.js`'s `setOnViewingPtChange` (fires on run-switch events like `endPlaythrough`/`loadRun`) and `play.js`'s `setAfterRenderFn` (fires at the end of *every* `render()` call). The second one is required because fast-travel jumps (`boot.js`'s `doJump`) and the sidebar's own `.choice-btn` navigation both push to `pt.path` and call `render()` directly without ever calling `setViewingPt()` - relying on the `setOnViewingPtChange` hook alone left the panel showing stale content whenever the player navigated by any means other than clicking a link inside the panel itself. `setAfterRenderFn` was a single-callback slot already owned by `dice.js`; it's now a list so both can register without clobbering each other.

`render()` fires far more often than the player actually changes section, so `_showSection()` tracks the currently-displayed section (`_shownSec`) and no-ops if `renderLiveRead()` asks for the same one again - without this, every render() (not just navigation) would re-fetch and reset `body.scrollTop`, yanking the panel back to the top out from under a scrolling reader. Reset to `undefined` on close so a later reopen always re-fetches (the panel could belong to a different book by then).

**Auto-nav suppression:** `play.js`'s `renderPlaythroughPanel()` auto-navigates through any section with exactly one already-known choice (a "straight path", no real decision to make). Reveal-on-arrival's `commitChoices()` call feeds that feature a freshly-discovered single choice on every straight-path section, which would silently fire `navigate()` in the background before the player ever sees that section's text in the panel. `liveread.js` calls `play.js`'s `suppressAutoNav(true)`/`(false)` on every panel open/close path (`_open`/`_close`, `setLiveReadVisible(false)`, `renderLiveRead()`'s no-playthrough fallback) so reading always shows every section, straight-path or not. `suppressAutoNav` is ref-counted internally (not a plain boolean) - `party.js` also wraps SSE-driven state syncs in the same calls, and a boolean would let whichever caller finishes first clear the other's still-active suppression via its own `finally` block.

**Validation checklist for a new (or re-imported) `book_sections` file** - a real broken link (book 868's chapter 28) shipped once from skipping item 5, and real OCR garbage (book 202's §366 and others) shipped once from skipping items 6-7 (structural checks alone say nothing about whether the prose itself is legible - see [[feedback_book_sections_prose_quality_check]]). All seven run on every import, not just the first:
...
**Books with `book_sections` data imported as of 2026-08-16:** 186 (Starship Traveller), 200 (The Forest of Doom), 201 (City of Thieves), 202 (Deathtrap Dungeon), 203 (Island of the Lizard King), 204 (Scorpion Swamp), 205 (Caverns of the Snow Witch), 206 (House of Hell), 207 (Talisman of Death), 208 (Space Assassin), 868 (Дугулите и нашествието на бумбаците).
...
**Several source-export tags had never been styled at all (fixed 2026-08-17), across books 200/204/209/210:** `<table>`/`.combat-card`/`.shop-table` (bare browser-default tables, plus an unconstrained table risks the exact same width-overflow bug `<pre>` had below, just via table auto-sizing instead of `white-space:pre` - fixed with `table-layout:fixed; width:100%`), `.choices` (a `<br>`-separated option list with no line spacing), `.listblock` (a deliberately line-broken quoted passage, e.g. an inscription, that needed framing so it doesn't read as a formatting glitch), and the `<p class="statline"|"stats"|"combat-note"|"source-gap">` variants of things already handled for `<pre>`/`<p class="note">`. `<pre class="statline"|"stats">` itself (found via a user screenshot: "one long line of text, not wrapping") is the more serious sibling bug - `<pre>` defaults to `white-space:pre`, which doesn't wrap long lines at all, unlike every other tag here; fixed with `white-space:pre-wrap` (keeps the source's own intentional line breaks, still wraps anything too long) plus resetting the inherited monospace font. All now share the same left-border "structural, not narrative" accent already used for `p.choice`/`p.end`/`p.note`.
...
**Gotcha: the global `document.addEventListener('click', hideCtxMenu)` (closes the context menu on any outside click) breaks anything that confirms asynchronously after its own opening click.** Clicking `#ctx-color-custom` to open the native OS color picker is itself a real click that bubbles to that document-level listener before the picker's own `change` event ever fires - `hideCtxMenu()` cleared `ctxNodeId` immediately on that first click, so by the time the user actually picked a color the handler read `ctxNodeId` as already `null` and silently no-op'd (fixed 2026-08-16 by adding a dedicated `click` listener on `#ctx-color-custom` that calls `e.stopPropagation()`, keeping the menu and `ctxNodeId` alive while the picker is open). Any future native-picker-style input added inside a context menu (date, file, etc.) needs the same guard.
...
- Migrates old path endings: if a path ends with -1 or 0 (pre-terminal-model data), pops it and sets `completed`/`result` instead, and if that pop left the path empty, defaults it to `[1]`. **That `[1]` fallback must stay nested inside the `isTerminal(last)` branch, never applied unconditionally to every playthrough regardless of path state** - it used to run for any playthrough with an empty path, which on every single book load stamped real-looking path content onto every untouched open-world series-run placeholder (`startedAt: null`) in that book, making every "is this book actually hosting this run" check that relied on path content think the placeholder was genuinely played. This was the deepest and most impactful of three independent bugs found in one session that all produced the same symptom (a phantom placeholder ending up looking touched) - see "Completion must only sync onto the book that actually ran it" and the isolated-portal-entry/`_handleNewSeriesRun` notes elsewhere in this doc for the other two. All downstream "is this touched" checks across the codebase (`_syncSeriesRuns`, `handleSaveState`, `processStateXp`, `_seriesRunBook`) now key off `pt.startedAt` alone, never `pt.path.length`, precisely because path content turned out to be spoofable by three separate pre-existing code paths.
...
Portal nodes are rendered as diamonds only in books that belong to an open world series. Non-open-world books are completely unaffected - the `nodeShape` logic in `graph.js` checks `_graphIsOpenWorld` before applying portal styling. Fill color follows the same `nodeColor()` rules as every other node (mapped/discovered/outcome) - portals previously got a hardcoded teal fill regardless of visited state, which never changed no matter what the player did, so a never-visited portal looked identical to a fully-explored one. A gold border (`#facc15`, independent of fill) keeps a portal easy to spot even when its fill blends in with a sea of same-colored mapped nodes. The in-app `#legend` (`index.html`) has a matching diamond-shaped "Portal" row, toggled visible/hidden by `setGraphOpenWorld()` in `graph.js` alongside the rest of the open-world graph setup.
...
Any code that reuses an *existing* playthrough slot (rather than creating a fresh one) and then pushes into its `path` must set `pt.startedAt` first if it isn't already set. `_syncSeriesRuns`'s own pad-to-`needed` loop always sets `startedAt` for slots it freshly creates, but `_handleNewSeriesRun`'s `while (state.playthroughs.length <= run_index)` loop only extends the array if it's *too short* - if a slot at `run_index` already exists as an untouched `_syncSeriesRuns` padding placeholder (`startedAt: null`), the loop does nothing and the function pushes the start section straight into that placeholder's path without ever stamping `startedAt`, exactly reproducing the same "phantom looks touched" bug as the `loadState`/isolated-portal-entry issues above. Fixed by stamping `startedAt` unconditionally (`if (!pt.startedAt) pt.startedAt = Date.now();`) right where `pt` is resolved, before touching `path` at all - matching the pattern `startPortalRun`/`doJumpCrossBook` already use.
...
- **`_syncSeriesRuns(seriesId)`:** called on every open-world book open. Fetches `GET /api/series/:id/runs` and propagates `completed`, `result`, `is_public` to local `state_data` playthroughs - but only onto a book whose own placeholder for that run index was actually touched (`pt.path.length > 0 || pt.startedAt`). Every book in the series carries a placeholder for every run index so numbers stay aligned, but only the book(s) the run actually visited may have their placeholder marked complete; copying completion onto an untouched placeholder made that book's own next save look like a genuine new completion to `handleSaveState` (see below), which re-awarded XP and re-stamped `series_runs.completed_at`. Edge cases: (a) local terminal playthrough with no `series_runs` record → pushes completion up; (b) active run's `last_section` absent from local path → restored; (c) surplus portal-paused or empty-placeholder runs beyond series run count → pruned (prevents phantom recreation after cross-book deletes).
- **`completeSeriesRun(seriesRunIndex, result)`:** called from `handleSaveState` on newly-terminal playthroughs. Writes `{ completed: true, result }` to `series_runs`, then calls `_syncSeriesRuns` to propagate immediately to all books. `handleSaveState` only treats a playthrough as newly-terminal if it also has real path content (`newPt.path?.length > 0`) - guards against a leaked/synced completion (see above) on a book the run never visited triggering a duplicate completion+XP award.
- **`processStateXp` (`server/db/xp.js`) has the same guard, independently.** Its own per-run loop (death_run/win_run/battle_run, first_loss/first_win, share_run, charsheet_saved/charsheet_run, add_charsheet_field) and its reconciliation safety net both skip any playthrough that isn't "touched" (`pt.path?.length > 0 || pt.startedAt`) before awarding anything for it - a leaked/stale placeholder still gets its `charSheet` mirrored onto it for display (`_syncSeriesRuns` sets `pt.charSheet = sr.char_data` unconditionally, outside the touched check, since the character is meant to travel with the series), so without this guard `add_charsheet_field` in particular could re-award XP for fields that only exist there because they were mirrored in, not genuinely added in this book. Uses `startedAt` (not `path.length`) as the alternate signal so a brand-new real run that hasn't made its first choice yet still earns day-one charsheet-edit XP.

**Pre-series runs:** the first time a book's series turns open-world, `migratePreSeriesRuns` (server, one-time) and `_syncSeriesRuns` (client, mirrors the same logic as a fallback) move any pre-existing runs out of `playthroughs` into `state_data.preSeriesRuns`, since open-world's own run slots need `playthroughs[i]` to line up index-for-index with `series_runs`. `activePtIndex` is reindexed (or nulled if the active run itself was migrated) rather than left dangling. `preSeriesRuns` still counts in `getProfileStats()`/`adminGetUsers()`, still generates `run_completed` (and `first_win`/`first_loss`/`first_battle_death` via `_resolveRunIndex`) entries in `getFeed()` - deliberately not gated on `seriesIsOpenWorld` like the `playthroughs` loop is, since these runs never get a `series_run_completed` entry either - and is still reachable for public viewing (`getPublicProfile`, `getPublicRun`, `getBookActivity`) via a negative `runIndex` - `-(preSeriesRuns.length - i)` for entry `i` - matching the client's own "Run -N" display convention (`play.js`). The URL route (`publicRunRe` in `server.js`) allows a leading `-` for this reason.

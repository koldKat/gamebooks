  ...
  public/
    ...
    js/
      ...
      battlesim/         All battle simulator modules, one file per book, grouped in their own
        ...
        battlesim286.js    Battle simulator for book 286 - flat weapon min-hit damage model (damage = max(0, 2d6 - minHit)).
        battlesim198.js    Battle simulator for book 198, The Warlock of Firetop Mountain - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat.
        battlesim199.js    Battle simulator for book 199, The Citadel of Chaos - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat, plus a MAGIC/spell system.
        battlesim200.js    Battle simulator for book 200, The Forest of Doom - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat.
        battlesim186.js    Battle simulator for book 186, Starship Traveller - multi-mode combat (hand-to-hand/phaser/ship-to-ship via a mode toggle), no LUCK-based combat swing.
        battlesim201.js    Battle simulator for book 201, City of Thieves - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat.
        battlesim202.js    Battle simulator for book 202, Deathtrap Dungeon - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat.
        battlesim203.js    Battle simulator for book 203, Island of the Lizard King - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat.
        battlesim246.js    Battle simulator for book 246, Return to Firetop Mountain - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat, plus a required-weapon/reduced-damage toggle (silver dagger fights) and an instant-kill-roll toggle (Undead Chaos Warrior/Doppelganger fights that don't wear STAMINA down). Full 45-entry enemy roster (every named slot of every multi-enemy encounter) seeded in book_enemies.
        battlesim234.js    Battle simulator for book 234, Vault of the Vampire - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat, plus a tracked FAITH stat, a sword-vs-Vampire SKILL bonus toggle, a continuous-drain/freeze-ray per-round side-effect toggle, and a three-way fight mode (single shared roll-off, highest Attack Strength lands the only blow - not the same mechanic as book 201's independent-paired-attacker toggle of the same name).
        battlesim231.js    Battle simulator for book 231, Daggers of Darkness - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat, plus darkness-fight/Necromancer-level/draw-threshold/double-1-drowns per-encounter toggles.
        battlesim232.js    Battle simulator for book 232, Armies of Death - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat; paired/side-enemy mechanic covers the Tree Man's simultaneous two-branch attack. Army/Skirmish-Battle troop management is out of scope (tracked on the Adventure Sheet, not a SKILL/STAMINA fight).
        battlesim716.js    Battle simulator for book 716, Арена 3 - not FF-style SKILL/STAMINA. Implements the book's own built-in "classic bojna sistema" (episode 50): АТАКА = СИЛА + one of БЪРЗИНА/ЛОВКОСТ (player's choice), ЗАЩИТА = РЕФЛЕКС + the other; each side lands БОЙНО МАЙСТОРСТВО hits per round, each hit = attacker's АТАКА + 1d6 minus defender's ЗАЩИТА, subtracted from ИЗДРЪЖЛИВОСТ. Enemies with 10 starting ИЗДРЪЖЛИВОСТ are "до първа кръв" (non-lethal) - the loser recovers half their lost points instead of dying. Roster: Докер, Бързака, Горилчо, Ю Чан, Ю Чен, Густав Хамър - all read from the book's own stat blocks. The climactic final battle against Юмо Унищожителя is out of scope (no symmetric fixed-stat block exists for it in the book, same as books 714/715's non-sim-applicable combat).
        battlesim83.js     Battle simulator for book 83, Войната на Понтиак / War of Pontiac - random-number-pick STRENGTH/LIFE combat.
        battlesim86.js     Battle simulator for book 86, Гората на демона / Forest of the Demon - random-number-pick STRENGTH/LIFE combat.
        battlesim114.js    Battle simulator for book 114, Огнена пустиня / Fiery Desert - random-number-pick STRENGTH/LIFE combat.
        battlesim115.js    Battle simulator for book 115, Окото на дявола / Eye of the Devil - random-number-pick STRENGTH/LIFE combat.
        battlesim123.js    Battle simulator for book 123, Прокълнатата земя / Damned Land - random-number-pick STRENGTH/LIFE combat.
        battlesim130.js    Battle simulator for book 130, Тайната на светещия мъх / Secret of the Glowing Moss - random-number-pick STRENGTH/LIFE combat.
        battlesim92.js     Battle simulator for book 92, Замъкът на таласъмите / Castle of the Goblins - table-driven combat lookup.
        battlesim108.js    Battle simulator for book 108, Ледените пирати / The Ice Pirates - table-driven combat lookup.
        battlesim193.js    Battle simulator for book 193, Flight from the Dark - Lone Wolf Combat Ratio / Combat Results Table lookup.
        battlesim217.js    Battle simulator for book 217, Trial of Champions - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat.
        battlesim526.js    Battle simulator for book 526, GrailQuest 1: The Castle of Darkness - single-roll LIFE POINTS system, no opposed-roll comparison.
        battlesim322.js    Battle simulator for book 322, Fire on the Water - Lone Wolf Combat Ratio / Combat Results Table lookup.
        battlesim323.js    Battle simulator for book 323, The Caverns of Kalte - Lone Wolf Combat Ratio / Combat Results Table lookup.
        battlesim324.js    Battle simulator for book 324, The Chasm of Doom - Lone Wolf Combat Ratio / Combat Results Table lookup.
        battlesim325.js    Battle simulator for book 325, Shadow on the Sand - Lone Wolf Combat Ratio / Combat Results Table lookup.
        battlesim122.js    Battle simulator for book 122, Проклятието на меча - switchable Battle-Code-based combat.
        battlesim80.js     Battle simulator for book 80, Бойците на Орм (The Fighters of Orm) - dice-based tournament combat.
        battlesim82.js     Battle simulator for book 82, Варварският бог (The Barbarian God) - random-number-pick STRENGTH/LIFE combat.
        battlesim118.js    Battle simulator for book 118, Полет от мрака - Lone Wolf Combat Ratio / Combat Results Table lookup.
        battlesim218.js    Battle simulator for book 218, Robot Commando - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat.
        battlesim734.js    Battle simulator for book 734, Командир на роботи - the Bulgarian translation of book 218 (Robot Commando), same mechanics/roster, Bulgarian UI text.
        battlesim739.js    Battle simulator for book 739, Майстори на меча - models the book's own "Проведи Схватка" mechanic (pick 1-6 for both sides, add Skill delta, apply to loser's life points); generic calculator, not tied to a single enemy, since the book is a tournament bracket with 7 scattered formal duel instances across 4 playable fighters' separate paths.
        ...
        battlesim220.js    Battle simulator for book 220, Creature of Havoc - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat.
        ...
        battlesim320.js    Battle simulator for book 320, Doomwalk (Blood Sword 4) - Fighting Prowess/Endurance Attack Strength combat.
        ...
        battlesim370.js    Battle simulator for book 370, Узурпатор! (Way of the Tiger 1) - technique-based combat (Hand/Kick/Throw + Shuriken resource, Inner Force).
        battlesim375.js    Battle simulator for book 375, Убиец! (Way of the Tiger 2) - same technique-based combat as book 370.
        battlesim376.js    Battle simulator for book 376, Властелин! (Way of the Tiger 3) - same technique-based combat as book 370.
        battlesim377.js    Battle simulator for book 377, Завоевател! (Way of the Tiger 4) - same technique-based combat as book 370.
        battlesim378.js    Battle simulator for book 378, Пъкъл! (Way of the Tiger 5) - same technique-based combat as book 370.
        ...
        battlesim206.js    Battle simulator for book 206, House of Hell - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat, player starts unarmed.
        battlesim207.js    Battle simulator for book 207, Talisman of Death - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat.
        battlesim208.js    Battle simulator for book 208, Space Assassin - multi-mode combat (hand-to-hand/gunfire via a mode toggle), plus an ARMOUR stat.
        battlesim209.js    Battle simulator for book 209, Freeway Fighter - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat.
        battlesim210.js    Battle simulator for book 210, Temple of Terror - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat.
        battlesim211.js    Battle simulator for book 211, The Rings of Kether - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat.
        battlesim212.js    Battle simulator for book 212, Seas of Blood - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat, shared between personal and crew-scale battles.
        battlesim213.js    Battle simulator for book 213, Appointment with F.E.A.R. - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat.
        battlesim214.js    Battle simulator for book 214, Rebel Planet - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat.
        battlesim215.js    Battle simulator for book 215, Demons of the Deep - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat.
        battlesim240.js    Battle simulator for book 240, Legend of the Shadow Warriors - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat, plus weapon (sword/battle-axe) and armour (none/chainmail/plate) choice affecting wound amounts, a generalized group-fight mode (up to 8 simultaneous side attackers, each with its own separate exchange), a disarm-fight mode (win by 3+ margin disarms instead of wounding, no STAMINA track), an "immune to wounds + roll for the skull" toggle for the Pan-Terric Behemoth, and Voivod-boss-specific mechanics (thrives-on-your-death STAMINA reflection, and the Spear of Doom's dual use as both a general auto-win item and its own defensive Life-force-roll mechanic in the sec. 171 fight).
        battlesim241.js    Battle simulator for book 241, Spectral Stalkers - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat with Provisions (heal 4, max 2), no potions or magic combat items (this book doesn't have any).
        battlesim242.js    Battle simulator for book 242, Tower of Destruction - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat with Provisions (heal 4, max 10). This book also tracks a fourth HONOUR stat, but HONOUR only affects narrative choices, never combat math, so the sim doesn't model it.
        battlesim243.js    Battle simulator for book 243, The Crimson Tide (Fighting Fantasy #47) - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat with Provisions (heal 4, max 10). This book also tracks a fourth FEROCITY stat, but FEROCITY only affects narrative dice choices and a FEROCITY<=0 story branch, never combat math, so the sim doesn't model it.
        battlesim244.js    Battle simulator for book 244, Moonrunner (Fighting Fantasy #48) - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat with Provisions (heal 4, max 5). One encounter (the Obisian Predator, sections 91/279) has no STAMINA score at all - it ends after a fixed round count or a Luck test rather than STAMINA reaching 0 - so it doesn't fit the normal win/loss loop and is left for the player to track by hand.
        battlesim245.js    Battle simulator for book 245, Siege of Sardath - Fighting Fantasy SKILL/STAMINA/LUCK opposed-roll combat with Bundles of Herbs (Herb Lore healing, heal 4, max 5, this book's name for the Provisions mechanic). 31 enemy stat blocks seeded into book_enemies, including paired/tripled encounters (Slykk, Toa-Suo, Black Flyer, Obligath, Elite Dark Elf) and two bosses (Morn Preeler, Thyra Migurn). Several encounters (Ghost of Khornu Wych's non-standard damage, Vampyric Slime Mould's colony mechanic, Istu/Pnea's double-demon fight, Morn Preeler's transformation) have narrative-specific rules beyond the standard duel loop and are left for the player to track by hand.
        battlesim753.js    Battle simulator for book 753, Вълшебната тетива - asymmetric binary-hit-counter combat (player's roll vs enemy DEFENSE either lands a hit or not, no variable damage; enemy's roll vs player DEFENSE adds excess to a 24-point ПОРАЖЕНИЯ pool). No formal stat-rolling rules exist in the book - ATAKA/ЗАЩИТА are freely editable, matching how the book itself recomputes them by hand per fight. Enemy roster fed from book_enemies via autocomplete, including a "Кобалди режим" toggle for the one fight with a dice-difference strikes-per-round formula instead of a fixed count.
        battlesim760.js    Battle simulator for book 760, Бойците на Европа - 5-fighter round-robin tournament plus a shared final boss (666); one uniform dice resolver (initiative = Interactive Status + d6, double-hit bonus at a 3+ gap, damage = Offensive Status + d6 minus defender's Defensive Status). Lets the reader pick any fighter vs any opponent rather than enforcing the book's bracket order.
        battlesim772.js    Battle simulator for book 772, Бойците на Кунг-Фу - round-based 1d6 dice combat (leg/hand/combined attack vs passive/aggressive defense, opponent-specific AI action patterns, "Отклоняване на получен удар" dodge-negation on some enemies). The protagonist's 17-stat build is entirely player-driven across 100+ branching training choices, so there's no canonical loadout - the player types in their own current stats rather than picking a preset character. Final antagonist Ян Лучан is excluded from the roster (every encounter with him is a scripted instant win/loss, never a real stat-based fight).
        battlesim781.js    Battle simulator for book 781, Мутирала плът - freeform weapon-vs-enemy combat (5 weapons with fixed bonus + relevant stat, some with limited ammo; a "defensive tactic" option reduces incoming damage by Физика instead of attacking). No canonical build (point-buy journal, reader-distributed), so the reader types in their own current stats. A narrative-only "momiche" mutant encounter is excluded from the roster (instant-death trap if attacked, not a real fight).
        battlesim869.js    Battle simulator for book 869, Тигрово око - СИЛА/ИЗДРЪЖЛИВОСТ/БОЙНИ УМЕНИЯ/МАГИЯ stat combat against 7 named rivals, each with its own two-phase (fresh vs. weakened) attack/defense formula tree plus a shared player attack menu and post-attack counter-check. No canonical build (stats/gear evolve through the journey), so the reader types in their own current numbers; the 7 rivals' book-listed stats/gear prefill as a convenience. Two rivals (Тейбрун, Матуал) replace their weakened-phase formula with the book's own one-off narrative event (truce offer / water-ambush trap) instead of another fight.
        battlesim871.js    Battle simulator for book 871, Сага за Ринглас - one uniform 2d6-vs-target resolver used for every fight in the book (Ринглас's target is always 6; each of 13 enemies has its own target/ИЗДРЪЖЛИВОСТ pair). Multi-enemy fights add +5 stamina to Ринглас (kept afterward, per the book's own rule) and an extra horse-attack roll each round. A couple of fights grant an item-based roll modifier (e.g. torch/scarves vs. wolves), exposed as optional bonus/penalty fields rather than tracked automatically.
        battlesim877.js    Battle simulator for book 877, Галактическият гигант - СИЛА/ЖИВОТ/ЕСПЕРНА МОЩ character (2d6/1d6+12/3d6), an encounter picker over the book's 12 genuine stat-based fights, each resolved with its own printed formula rather than a uniform mechanic (some are pure stat comparisons with no dice, two are multi-round dice duels, a few roll once against a threshold).
        battlesim881.js    Battle simulator for book 881, Дракон в мазето - СИЛА/ИЗДРЪЖЛИВОСТ character (starts 5/30) using the book's own printed round-based combat rule (die + СИЛА vs enemy's fixed СИЛА, winner deals 2 points, ties cost both 1), a 3-way interchangeable sword (Фирфелд/Лайм/Истрин) each applying its own per-encounter СИЛА bonus/penalty, and a 16-fight encounter picker covering every genuine stat-based fight in the book. Two encounters (a 3-kill wolf pack, a single opposed knife-throw duel) use their own one-off printed mechanic instead of the default round-robin resolver. The final dragon fight is deliberately excluded - every combat attempt against it is a scripted instant death, the only real win path is a name-guessing puzzle plus diplomacy.
        battlesim882.js    Battle simulator for book 882, Тайната на Зоро - implements all three of the book's own printed combat systems (Ръкопашна схватка hand-to-hand, Схватка с шпаги group sword fights, and formal 5-stat Дуел with Пронизващ удар/Сечащ удар/Блок/Финт/Трикове and a Трикове-driven series-interruption mechanic), a full 13-stat character sheet, and a 16-encounter picker spanning all three systems.
        battlesim236.js    Battle simulator for book 236, Dead of Night - standard Fighting Fantasy SKILL/STAMINA/LUCK combat (2d6+SKILL Attack Strength, 2 STAMINA per wound), plus book-specific toggles for the Demon-Slaying sword (+4 SKILL vs Demons), the Armoured Ogre's thick armour (1 STAMINA per landed hit instead of 2), the Potion of Heroism (one-time +2 STAMINA/+2 SKILL), and the final Myurr fight (two enemy attacks per round, only a magical weapon can wound him).
        battlesim237.js    Battle simulator for book 237, Master of Chaos - standard Fighting Fantasy SKILL/STAMINA/LUCK combat, with generic attack-modifier and configurable damage-per-hit fields (covering this book's many one-off temporary SKILL penalties and non-standard damage amounts without hardcoding each), plus an optional second-enemy toggle implementing the book's "highest Attack Strength of the three combatants wins the round" simultaneous-fight mechanic (Skeletons sec.109, Crewmen sec.225, Gnomes sec.293). The book's many "fight them one at a time" pairs need no special code - just the standard single-enemy engine run twice.
...
**Request-target validation:** `server/request-url.js` validates every incoming request target
before authentication, impersonation context setup, or route dispatch. Only valid origin-form
paths are accepted. Authority-form targets, backslashes, control characters, malformed URLs,
and targets that resolve away from the fixed internal parsing origin receive HTTP 400. Query
token extraction uses the same parser and fails closed, preventing malformed scanner probes
such as `//%2F.env` from throwing outside the router and terminating the Node process. The
outer handler also catches rejected routing promises so an unexpected pre-response failure is
logged and returned as HTTP 500 instead of becoming an unhandled rejection.
...
| `undo` | 1 | `bookId:stamp:u:i:seq` | Per undo performed. `processStateXp` diffs the summed `undosUsed` counter across all playthroughs between old and new state (the counter play.js bumps on each undo), awarding one unit of XP per increment. Ref carries the save's timestamp, loop index, and a process-lifetime sequence so a reset-progress zeroing of the counters can't collide with already-logged refs. Counters are client-supplied, so inflation is possible - accepted deliberately at this amount (same stance as battlesim_win/loss: spam pays less than playing). Uses before the counters existed and uses in since-reset books were never recorded; pre-payout history was backfilled once as a single lump award per user with ref `retro_undo` via `awardXp`'s amountOverride. |
| `fast_travel` | 2 | `bookId:stamp:f:i:seq` | Per fast travel performed. Identical mechanism to `undo`, diffing `fastTravelsUsed` (boot.js / open-world.js bump it); same backfill exists with ref `retro_fast_travel`. |
...
- `GET /api/feed` is a pure read with no side effects; it does not award `idle_heartbeat` XP. On desktop, heartbeat XP only comes from `livetab.js`'s dedicated 60-second leader-tab timer hitting `POST /api/heartbeat` directly - see `idle_heartbeat` in the XP events table above. `loadFeed()` still schedules a short profile refresh after every call (`scheduleRewardProfileRefresh(150)`) so the coin/XP floater picks up whatever the heartbeat timer most recently did. The heartbeat endpoint responds with `{ ok, awarded, coinRolled }`; the sender schedules that same profile refresh whenever the response reports an award (or a freshly rolled bonus coin), so overnight accrual surfaces in the coins display/coin button/XP bar without needing a feed reload. Tab wake (`visibilitychange` to visible, or window focus) also re-runs the feed-version check and schedules the same refresh, catching up a tab whose timers were throttled or whose SSE connections died while backgrounded.
...
**`loadFeed()` is single-flight.** The exported `loadFeed()` is a thin wrapper around `_loadFeedImpl()` that reuses the in-flight promise if one is already running, rather than starting a second full DOM rebuild - `livetab.js`'s 60s leader-tab poll and an SSE-triggered `feed_changed` refresh can land in the same tick. `livetab.js`'s poll itself also skips calling `loadFeed()` entirely while `#main-screen` is showing (a book/graph is open) - the feed panel's own toggle button is hidden in that state (`boot.js`'s `showMain()`), so there's nothing to rebuild for; `showBooks()` calls `loadFeed()` directly on return, so nothing goes stale beyond the interval's own cadence. On ticks where the landing IS visible, the poll first fetches the cheap `GET /api/feed/version` fingerprint - per-source `COUNT`/`MAX` aggregates in `server/db/feed.js`'s `getFeedVersion()` mirroring `getFeed()`'s own sources (a whitelist of feed-visible `xp_events` types, so per-minute awards like `idle_heartbeat` don't defeat it), plus the calendar date so day labels roll over at midnight - and only calls `loadFeed()` when the fingerprint moved; a version-fetch failure falls back to the unconditional reload. Reloads triggered outside the gated tick (SSE `feed_changed` events, the dirty-flag catch-up on becoming visible/focus, `boot.js`'s direct `loadFeed()` in `showLogin()`/`showBooks()`) re-baseline the stored fingerprint so the next tick doesn't rebuild the just-rendered change a second time.
...
`_assignLocalPositions()` (auto-placement for newly-added nodes on a book that already has a saved layout, in `graph.js`) first offers any new node with a positioned incoming parent a directional slot: one `_GRID_LAYER_GAP` column to that parent's right, siblings stacking downward in the column (nodes with no positioned parent fall through to `_chooseLocalPosition()`'s radial overlap scoring). It also snaps its output when `state.snapToGrid` is on, after the placement runs against the real, unrounded neighbor positions - not before, so rounding doesn't skew the scoring itself. A freshly-added node was never hand-placed by the user, so this doesn't conflict with "snap never retroactive"; there's no existing deliberate placement to disturb. The one accepted trade-off: rounding can occasionally land a new node a little closer to a neighbor than the scoring intended, same as the drag-end snap already accepts.
...
For each missing node, `_assignGridPositions()` places it one grid column past its positioned incoming parents - falling back to any positioned neighbor, then to raw `depth * _GRID_LAYER_GAP` only when no neighbor is positioned - processing missing nodes in ascending BFS-depth order so parents are positioned before their children. Both the column (x) and the starting stack height (y) anchor on the same neighbor: siblings fill the first free slot stepping DOWN from the anchor's own y (`_firstFreeColumnSlot`), never a whole-map bottom-of-column scan, so a fresh option lands beside the node just stepped on rather than below unrelated branches in the same column band. The same applies in the non-grid regime: `_assignLocalPositions()` tries `_chooseDirectionalPosition()` first (max-x positioned incoming parent, one column right, same local slot search) and only falls back to the radial overlap-scoring `_chooseLocalPosition()` for nodes with no positioned parent. When `state.snapToGrid` is on, the placed x/y are rounded to `GRID_SIZE`. Mobile's `_layout()` has the equivalent logic via its own local `_positionedNeighbors()` and `_firstFreeRowSlot()` (same fix, axes swapped) and always rounds its placements to `GRID_SIZE` (mobile has no snap toggle - drags always snap). Every node the grid places gets `physics: false`. `_assignGridPositions` only ever fills gaps, never moves an existing position.
...
**Section-id 0 is unusable:** `isTerminal()` reserves numeric `0`/`-1` as the win/death sentinels, so a book whose own numbering starts at "chapter 0" can't have a `book_sections` row for it - `0` can never be a real navigable `state.graph` node. The import folds chapter 0's prose into the start of chapter 1 instead of dropping it, stripping chapter 0's own (now-redundant) "turn to chapter 1" transition first so it doesn't survive as a nonsensical self-referencing choice.
...
5. No un-linked plain-text "turn to N" mention anywhere in a section's body - strip existing `<a>` tags first, then regex for the book's own turn-to phrasing, matching verb-form variants too, not just "turn to". This is the one that's easy to skip because it doesn't fail on *structure* (ids/refs/counts all look fine), only on *content* a human has to actually read to catch.
...
**Genuine source gaps:** sometimes the supplied scan itself is missing a section - the source jumps straight from one number to another with no intervening page. This isn't an extraction bug and shouldn't be patched by inventing prose. The correct handling is a `class="section gap"` stub with an explicit `[SOURCE GAP: ...]` placeholder paragraph and no choice links out of it; checklist item 2 above is satisfied because the stub section id still exists as a valid link target, it just has no real content or outgoing choices of its own.
...
**Combat stat blocks aren't always flattened text.** An enemy stat block may be exported as a real `<div class="combat-card"><table>...</table></div>` (columns: Enemy/SKILL/STAMINA) instead of the plain-text "NAME SKILL N STAMINA N" pattern - a structured table is easier to cross-verify programmatically (exact `data-enemy`/`.skill`/`.stamina` extraction) than regex-guessing at prose.
...
**Pre-series runs:** the first time a book's series turns open-world, `migratePreSeriesRuns` (server, one-time, runs once across the whole DB when the feature launched) moves any pre-existing runs out of `playthroughs` into `state_data.preSeriesRuns`, since open-world's own run slots need `playthroughs[i]` to line up index-for-index with `series_runs`. `_syncSeriesRuns` (client) mirrors the same eligibility logic as an ongoing fallback - unlike the server migration, this one is **not** one-time; it re-sweeps on every sync, not just the first. It used to be guarded by `state.preSeriesRuns === undefined`, but that left any run added after the first sweep sitting in plain `playthroughs` forever (still genuinely pre-series, since no real series run had registered yet), which the `needed`/`current` reconciliation below then misread as "extra" runs needing individual create-run API calls - retroactively registering each as a brand-new run today and spamming the feed with "began series run N" entries. `activePtIndex` is reindexed (or nulled if the active run itself was migrated). `preSeriesRuns` still counts in `getProfileStats()`/`adminGetUsers()`, still generates `run_completed` (and `first_win`/`first_loss`/`first_battle_death` via `_resolveRunIndex`) entries in `getFeed()` (not gated on `seriesIsOpenWorld` like the `playthroughs` loop is), and is still reachable for public viewing (`getPublicProfile`, `getPublicRun`, `getBookActivity`) via a negative `runIndex` - `-(preSeriesRuns.length - i)` for entry `i` - matching the client's own "Run -N" display convention (`play.js`). The URL route (`publicRunRe` in `server.js`) allows a leading `-` for this reason.

...
Layer 3 (feature modules - import from layers 0–2 as needed):
  notes.js, battlesim829.js, battlesim8.js, battlesim286.js, battlesim198.js, battlesim199.js, battlesim200.js, battlesim186.js, battlesim201.js, battlesim202.js, battlesim203.js, battlesim204.js, battlesim205.js, battlesim206.js, battlesim207.js, auth.js, add-book.js, edit-book.js,
...
A self-contained POC module that renders a book's actual prose section-by-section inside the play area, with clickable in-text choices. Distinct from everything else in the app: it's the only feature that stores real book prose server-side (`book_sections` table) rather than purely structural/mechanical/user-generated state.

**Gating:** server-side only, by username - `db._canLiveRead(userId)` hardcodes a small allowlist (`_LIVE_READ_USERNAMES`, currently koldKat + sashii). `getBooks()` returns `hasLiveReading: canLiveRead && !!has_live_reading` (computed per-viewer, never leaks to other readers of a book that has data even though the book's own flag is `true` for everyone). `GET /api/books/:id/sections/:secId` (`handleGetBookSection`) re-checks the same gate and 403s otherwise. Client-side, `boot.js`'s book-switch call site reads the server-computed `hasLiveReading` flag off the cached book record (`_bk?.hasLiveReading`) - no separate client-side username or book-ID hardcoding, unlike the per-book battlesim pattern (`setSimNNNVisible(bookId === N)`).

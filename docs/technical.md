...
**Forum dialog layout:** the `.wrap` container inside `forum.js` uses `max-width: 100%; padding: 0 0.5rem` (previously `max-width: 640px; margin: 0 auto`). This removes excessive horizontal padding inside the forum modal when it is rendered in a narrow container.

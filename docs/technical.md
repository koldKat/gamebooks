...
`server/runtime-state.js`'s `computeCodeStats()` (feeds "Lines of code"/"Code size"/"JS modules" in Stats for Nerds) scans `server/`, `public/js/`, `admin/js/`, `test/`, `public/mobile/js/`, `public/css/`, `public/mobile/css/`, plus `server.js` and the top-level HTML files (`public/index.html`, `public/guide.html`, `public/mobile/index.html`, `admin/*.html`), dynamically - not a hardcoded file list, so a new file needs a server restart to be picked up, not a code change here. `walkJsFiles()` matches both `.js` and `.mjs` (so `test/*.test.mjs` counts). `test/` and `public/mobile/` each have their own inner `try/catch`, separate from the outer one wrapping the rest of the function, so either directory going missing degrades that one count to zero instead of silently zeroing every stat in the panel.
...
- `public/mobile/js/battlesim-dispatch.js`'s `SIMS` table (`{path, init, btn}` keyed by `book_id`) - omitting a sim here means mobile's Battle Sim button just never appears for that book, no error. Independent version numbering from `boot.js`'s own import of the same file (each importer bumps its own counter).
...
**Gotcha: the global `document.addEventListener('click', hideCtxMenu)` (closes the context menu on any outside click) breaks anything that confirms asynchronously after its own opening click.** Clicking `#ctx-color-custom` to open the native OS color picker is itself a real click that bubbles to that document-level listener before the picker's own `change` event ever fires - `hideCtxMenu()` clearing `ctxNodeId` immediately on that first click would make the handler read `ctxNodeId` as already `null` by the time a color is actually picked, silently no-op'ing - guarded by a dedicated `click` listener on `#ctx-color-custom` that calls `e.stopPropagation()`, keeping the menu and `ctxNodeId` alive while the picker is open. Any future native-picker-style input added inside a context menu (date, file, etc.) needs the same guard.
...
Any code that reuses an *existing* playthrough slot (rather than creating a fresh one) and then pushes into its `path` must set `pt.startedAt` first if it isn't already set - `_handleNewSeriesRun` stamps it unconditionally (`if (!pt.startedAt) pt.startedAt = Date.now();`) right where `pt` is resolved, before touching `path` at all, matching the pattern `startPortalRun`/`doJumpCrossBook` use. This matters because `_syncSeriesRuns`'s own pad-to-`needed` loop can leave a slot at `run_index` as an untouched placeholder (`startedAt: null`) that `_handleNewSeriesRun`'s own array-extension loop wouldn't otherwise touch, since that loop only extends the array when it's too short.

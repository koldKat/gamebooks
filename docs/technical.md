...
`adminGetUsers()` (`server/db/admin.js`) and `getProfileStats()` (`server/db/feed.js`) both count a user's completed runs the same way: only entries whose `result` is `death`/`success`/`battle`, never in-progress runs. `adminGetUsers()` separately counts in-progress runs (`!pt.result`) as `active`, shown in the users table's own "Active" column - `runs + active` is the same total the individual-user detail page's `totalRuns` shows for the same user (that page counts every playthrough regardless of result), just split into the two categories. All four of these admin aggregates (`adminGetUsers()`'s `active`, `adminGetBookStats()`'s `inProgress`/`totalPts`, `adminGetUserBooks()`'s `playthroughs`, and `adminGetStats()`'s site-wide `activePlaythroughs`/`playthroughs`) exclude untouched open-world series-run placeholders (`pt.startedAt == null`) - every book in a series carries one padding slot per series run so numbers line up across books, and without this filter each of these counts was inflated by however many sibling books' padding slots the user's account happened to have, regardless of whether they'd ever actually played there.
